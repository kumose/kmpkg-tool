...
namespace
{
    ...
    struct FuzzArgs
    {
        ...
        [[noreturn]] void print_help_and_exit(bool invalid = false) const
        {
            ...
            StringLiteral message = "usage: kmpkg-fuzz --kind=<kind>\n\n"
    ...
    [[noreturn]] void fuzz_json_and_exit(StringView text)
    {
        ...
        if (!res)
        {
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, res.error());
        }
    ...
    [[noreturn]] void fuzz_platform_expr_and_exit(StringView text)
    {
        ...
        if (!res1)
        {
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, res1.error());
        }
        if (!res2)
        {
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, res2.error());
        }
