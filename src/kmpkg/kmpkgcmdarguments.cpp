...
namespace
{
    ...
    constexpr CIRecord KNOWN_CI_VARIABLES[]{
        ...
        {EnvironmentVariableKmpkgNoCi, "KMPKG_NO_CI", CIKind::None},
...
namespace kmpkg
{
    ...
    static void set_from_feature_flag(const std::vector<std::string>& flags, StringView flag, Optional<bool>& place)
    {
        if (!place.has_value())
        {
            ...
            if (std::find_if(flags.begin(), flags.end(), not_flag) != flags.end())
            {
                if (place.has_value())
                {
                    Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgTwoFeatureFlagsSpecified, msg::value = flag);
                }
    ...
    static void parse_feature_flags(const std::vector<std::string>& flags, KmpkgCmdArguments& args)
    {
    ...
    KmpkgCmdArguments KmpkgCmdArguments::create_from_command_line(const ILineReader& fs,
                                                                  const int argc,
                                                                  const CommandLineCharType* const* const argv)
    {
        std::vector<std::string> v = convert_argc_argv_to_arguments(argc, argv);
        replace_response_file_parameters(v, fs).value_or_exit(KMPKG_LINE_INFO);
        return KmpkgCmdArguments::create_from_arg_sequence(v.data(), v.data() + v.size());
    }

    KmpkgCmdArguments KmpkgCmdArguments::create_from_arg_sequence(const std::string* arg_first,
                                                                  const std::string* arg_last)
    {
        KmpkgCmdArguments args{CmdParser{View<std::string>{arg_first, arg_last}}};
        ...
        args.parser.parse_option(
            ...
            args.kmpkg_root_dir_arg,
            msg::format(msgKmpkgRootsDir, msg::env_var = format_environment_variable(EnvironmentVariableKmpkgRoot)));
        args.parser.parse_option(
            SwitchTriplet,
            StabilityTag::Standard,
            args.triplet,
            msg::format(msgSpecifyTargetArch,
                        msg::env_var = format_environment_variable(EnvironmentVariableKmpkgDefaultTriplet)));
        args.parser.parse_option(
            SwitchHostTriplet,
            StabilityTag::Standard,
            args.host_triplet,
            msg::format(msgSpecifyHostArch,
                        msg::env_var = format_environment_variable(EnvironmentVariableKmpkgDefaultHostTriplet)));
        args.parser.parse_switch(
        ...
        args.parser.parse_option(
            ...
            msg::format(msgDownloadRootsDir,
                        msg::env_var = format_environment_variable(EnvironmentVariableKmpkgDownloads)));
        args.parser.parse_option(
        ...
        args.parser.parse_multi_option(
            ...
            msg::format(msgOverlayPortsHelp,
                        msg::env_var = format_environment_variable(EnvironmentVariableKmpkgOverlayPorts)));
        args.parser.parse_multi_option(
    ...
    ParsedArguments KmpkgCmdArguments::parse_arguments(const CommandMetadata& command_metadata) const
    {
        ParsedArguments output;
    ...
    const std::vector<std::string>& KmpkgCmdArguments::get_forwardable_arguments() const noexcept
    {
        return forwardable_arguments;
    }

    KmpkgCmdArguments::KmpkgCmdArguments(CmdParser&& parser_) : parser(std::move(parser_)) { }
    ...
    {
        auto with_common_options = KmpkgCmdArguments::create_from_arg_sequence(nullptr, nullptr);
    ...
    void KmpkgCmdArguments::imbue_from_environment() { imbue_from_environment_impl(&kmpkg::get_environment_variable); }
    void KmpkgCmdArguments::imbue_from_fake_environment(const std::map<StringLiteral, std::string, std::less<>>& env)
    {
    ...
    void KmpkgCmdArguments::imbue_from_environment_impl(std::function<Optional<std::string>(ZStringView)> get_env)
    {
        if (!disable_metrics)
        {
            const auto kmpkg_disable_metrics_env = get_env(EnvironmentVariableKmpkgDisableMetrics);
            if (kmpkg_disable_metrics_env.has_value())
            {
                disable_metrics = true;
            }
        }

        from_env(get_env, EnvironmentVariableKmpkgDefaultTriplet, triplet);
        from_env(get_env, EnvironmentVariableKmpkgDefaultHostTriplet, host_triplet);
        kmpkg_root_dir_env = get_env(EnvironmentVariableKmpkgRoot);
        from_env(get_env, EnvironmentVariableKmpkgDownloads, downloads_root_dir);
        from_env(get_env, EnvironmentVariableXKmpkgAssetSources, asset_sources_template_env);
        from_env(get_env, EnvironmentVariableXKmpkgRegistriesCache, registries_cache_dir);
        from_env(get_env, EnvironmentVariableKmpkgVisualStudioPath, default_visual_studio_path);
        from_env(get_env, EnvironmentVariableKmpkgBinarySources, env_binary_sources);
        from_env(get_env, EnvironmentVariableXKmpkgNuGetIDPrefix, nuget_id_prefix);
        use_nuget_cache = get_env(EnvironmentVariableKmpkgUseNuGetCache).map([](const std::string& s) {
            return Strings::case_insensitive_ascii_equals(s, "true") || s == "1";
        });
        from_env(get_env, EnvironmentVariableKmpkgNuGetRepository, kmpkg_nuget_repository);
        ...
        {
            const auto kmpkg_disable_lock = get_env(EnvironmentVariableXKmpkgIgnoreLockFailures);
            if (kmpkg_disable_lock.has_value() && !ignore_lock_failures.has_value())
            {
        ...
        {
            const auto kmpkg_overlay_ports_env = get_env(EnvironmentVariableKmpkgOverlayPorts);
            if (const auto unpacked = kmpkg_overlay_ports_env.get())
            {
                env_overlay_ports = Strings::split_paths(*unpacked);
            }
        }
        {
            const auto kmpkg_overlay_triplets_env = get_env(EnvironmentVariableOverlayTriplets);
            if (const auto unpacked = kmpkg_overlay_triplets_env.get())
            {
                env_overlay_triplets = Strings::split_paths(*unpacked);
            }
        }
        {
            const auto kmpkg_feature_flags_env = get_env(EnvironmentVariableKmpkgFeatureFlags);
            if (const auto v = kmpkg_feature_flags_env.get())
            {
    ...
    void KmpkgCmdArguments::imbue_or_apply_process_recursion(KmpkgCmdArguments& args)
    {
        static bool s_reentrancy_guard = false;
        Checks::check_exit(
            KMPKG_LINE_INFO,
            !s_reentrancy_guard,
            "KmpkgCmdArguments::imbue_or_apply_process_recursion() modifies global state and thus may only be "
            "called once per process.");
        s_reentrancy_guard = true;

        auto maybe_kmpkg_recursive_data = get_environment_variable(EnvironmentVariableXKmpkgRecursiveData);
        if (auto kmpkg_recursive_data = maybe_kmpkg_recursive_data.get())
        {
            auto rec_doc = Json::parse(*kmpkg_recursive_data, EnvironmentVariableXKmpkgRecursiveData)
                               .value_or_exit(KMPKG_LINE_INFO)
                               .value;
            const auto& obj = rec_doc.object(KMPKG_LINE_INFO);

            if (auto entry = obj.get(JsonIdKmpkgRootArg))
            {
                auto as_sv = entry->string(KMPKG_LINE_INFO);
                args.kmpkg_root_dir_arg.emplace(as_sv.data(), as_sv.size());
            }

            if (auto entry = obj.get(JsonIdKmpkgRootEnv))
            {
                auto as_sv = entry->string(KMPKG_LINE_INFO);
                args.kmpkg_root_dir_env.emplace(as_sv.data(), as_sv.size());
            }

            if (auto entry = obj.get(JsonIdKmpkgDownloads))
            {
                auto as_sv = entry->string(KMPKG_LINE_INFO);
                args.downloads_root_dir.emplace(as_sv.data(), as_sv.size());
            }

            if (auto entry = obj.get(JsonIdKmpkgAssetSources))
            {
                auto as_sv = entry->string(KMPKG_LINE_INFO);
                args.asset_sources_template_env.emplace(as_sv.data(), as_sv.size());
            }

            if (obj.get(JsonIdKmpkgDisableMetrics))
            {
            ...
            set_environment_variable(EnvironmentVariableXKmpkgRecursiveData, "poison");
        }
        else
        {
            Json::Object obj;
            ...
            {
                obj.insert(JsonIdKmpkgRootArg, Json::Value::string(*kmpkg_root_dir_arg));
            }

            if (auto kmpkg_root_dir_env = args.kmpkg_root_dir_env.get())
            {
                obj.insert(JsonIdKmpkgRootEnv, Json::Value::string(*kmpkg_root_dir_env));
            }

            if (auto downloads_root_dir = args.downloads_root_dir.get())
            {
                obj.insert(JsonIdKmpkgDownloads, Json::Value::string(*downloads_root_dir));
            }

            auto maybe_ast = args.asset_sources_template();
            if (auto ast = maybe_ast.get())
            {
                obj.insert(JsonIdKmpkgAssetSources, Json::Value::string(*ast));
            }

            if (args.disable_metrics)
            {
                obj.insert(JsonIdKmpkgDisableMetrics, Json::Value::boolean(true));
            }

            set_environment_variable(EnvironmentVariableXKmpkgRecursiveData,
                                     Json::stringify(obj, Json::JsonStyle::with_spaces(0)));
        }
    }

    void KmpkgCmdArguments::check_feature_flag_consistency() const
    {
        struct
        {
            StringView flag;
    ...
    void KmpkgCmdArguments::debug_print_feature_flags() const
    {
        struct
        {
            StringView name;
    ...
    void KmpkgCmdArguments::track_feature_flag_metrics() const
    {
    ...
    void KmpkgCmdArguments::track_environment_metrics() const
    {
        MetricsSubmission submission;
    ...
    Optional<std::string> KmpkgCmdArguments::asset_sources_template() const
    {
