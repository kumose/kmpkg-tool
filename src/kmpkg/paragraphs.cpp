...
namespace kmpkg
{
    ...
    static Optional<std::vector<T>> parse_list_until_eof(Message bad_comma_message, ParserBase& parser, F f)
    {
        ...
        do
        {
            ...
            ret.push_back(std::move(item).value_or_exit(KMPKG_LINE_INFO));
            parser.skip_whitespace();
                                                                    ...
                                                                    TextRowCol textrowcol)
    {
        ...
        return {std::move(opt).value_or_exit(KMPKG_LINE_INFO), expected_left_tag};
    }
...
namespace kmpkg::Paragraphs
{
    ...
    PortLoadResult try_load_port(const ReadOnlyFilesystem& fs, const PortLocation& port_location)
    {
        ...
        auto manifest_path = port_location.port_directory / "kmpkg.json";
    ...
    {
        std::string spdx_location = "git+https://github.com/kumose/kmpkg#ports/";
    ...
    {
        std::string spdx_location = "git+https://github.com/kumose/kmpkg@";
    ...
    {
        LoadResults ret;
        std::vector<std::string> ports = registries.get_all_reachable_port_names().value_or_exit(KMPKG_LINE_INFO);
        for (const auto& port_name : ports)
        {
            ...
            auto maybe_baseline_version = impl->get_baseline_version(port_name).value_or_exit(KMPKG_LINE_INFO);
