...
namespace kmpkg
{
    ...
    void command_autocomplete_and_exit(const KmpkgCmdArguments& args, const KmpkgPaths& paths)
    {
        ...
        if (command_arguments.size() <= 1)
        {
            ...
            output_sorted_results_and_exit(KMPKG_LINE_INFO, std::move(results));
        }

        // command_arguments.size() >= 2
        const auto& command_name = command_arguments[0];

        // Handles kmpkg install package:<triplet>
        if (Strings::case_insensitive_ascii_equals(command_name, "install"))
        {
            ...
            if (colon != last_arg.end())
            {
                ...
                output_sorted_results_and_exit(KMPKG_LINE_INFO, std::move(result));
            }
        }
