...
namespace
{
    ...
    void xml_test(XmlSerializer& xml, const XunitTest& test)
    {
        ...
        {
            Checks::check_exit(KMPKG_LINE_INFO, result_string == "Pass");
        }
...
namespace kmpkg
{
    ...
    {
        out_str.append(kmpkg::to_string(code).data());
        if (auto b = build.get())
        {
            out_str.append(": ");
    ...
    std::string XunitWriter::build_xml(Triplet controlling_triplet) const
    {
        ...
        for (const auto& test_group : m_tests)
        {
            ...
            const auto as_tm = to_utc_time(as_time_t).value_or_exit(KMPKG_LINE_INFO);
