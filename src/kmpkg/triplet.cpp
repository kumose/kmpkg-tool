...
{
    template<>
    struct hash<kmpkg::TripletInstance>
    {
        size_t operator()(const kmpkg::TripletInstance& t) const { return t.hash; }
    };
}
...
namespace kmpkg
{
    ...
    Triplet default_triplet(const KmpkgCmdArguments& args, const TripletDatabase& database)
    {
        ...
        {
            check_triplet(*triplet_name, database).value_or_exit(KMPKG_LINE_INFO);
    ...
    Triplet default_host_triplet(const KmpkgCmdArguments& args, const TripletDatabase& database)
    {
        ...
        check_triplet(host_triplet_name, database).value_or_exit(KMPKG_LINE_INFO);
    ...
    Path TripletDatabase::get_triplet_file_path(Triplet triplet) const
    {
        ...
        if (it == available_triplets.end())
        {
            Checks::msg_exit_with_message(
                KMPKG_LINE_INFO, msgTripletFileNotFound, msg::triplet = triplet.canonical_name());
        }
