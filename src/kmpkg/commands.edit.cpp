...
namespace
{
    ...
    std::string expand_environment_strings(const std::string& input)
    {
        ...
        do
        {
            ...
            {
                Checks::exit_fail(KMPKG_LINE_INFO); // integer overflow
            }
    ...
    std::vector<std::string> create_editor_arguments(const KmpkgPaths& paths,
                                                     ...
                                                     const std::vector<std::string>& ports)
    {
        if (Util::Sets::contains(options.switches, SwitchAll))
        {
            ...
            auto packages = fs.get_files_non_recursive(paths.packages(), KMPKG_LINE_INFO);
...
namespace kmpkg
{
    constexpr CommandMetadata CommandEditMetadata{
        ...
        {msgCmdEditExample1, "kmpkg edit zlib"},
        Undocumented,
    ...
    void command_edit_and_exit(const KmpkgCmdArguments& args, const KmpkgPaths& paths)
    {
        ...
        Checks::exit_with_code(KMPKG_LINE_INFO, static_cast<int>(cmd_execute(cmd).value_or_exit(KMPKG_LINE_INFO)));
    }
} // namespace kmpkg
