...
StringView kmpkg::extract_first_cmake_invocation_args(StringView content, StringView command)
{
    // command\s*\(([^)]+)\)
...
StringView kmpkg::extract_arg_from_cmake_invocation_args(StringView invocation_args, StringView target_arg)
{
...
std::string kmpkg::replace_cmake_var(StringView text, StringView var, StringView value)
{
...
static void find_all_github(StringView text, Json::Array& packages, StringView version_text)
{
    ...
    while (it != text.end())
    {
        auto github = extract_first_cmake_invocation_args(StringView{it, text.end()}, "kmpkg_from_github");
        if (github.empty())
        {
            it = text.end();
            continue;
...
static void find_all_bitbucket(StringView text, Json::Array& packages, StringView version_text)
{
    ...
    while (it != text.end())
    {
        auto bitbucket = extract_first_cmake_invocation_args(StringView{it, text.end()}, "kmpkg_from_bitbucket");
        if (bitbucket.empty())
        {
            it = text.end();
            continue;
...
static void find_all_gitlab(StringView text, Json::Array& packages, StringView version_text)
{
    ...
    while (it != text.end())
    {
        auto gitlab = extract_first_cmake_invocation_args(StringView{it, text.end()}, "kmpkg_from_gitlab");
        if (gitlab.empty())
        {
            it = text.end();
            continue;
...
static void find_all_git(StringView text, Json::Array& packages, StringView version_text)
{
    ...
    while (it != text.end())
    {
        auto git = extract_first_cmake_invocation_args(StringView{it, text.end()}, "kmpkg_from_git");
        if (git.empty())
        {
            it = text.end();
            continue;
...
static void find_all_distfile(StringView text, Json::Array& packages, StringView version_text)
{
    ...
    while (it != text.end())
    {
        auto distfile = extract_first_cmake_invocation_args(StringView{it, text.end()}, "kmpkg_download_distfile");
        if (distfile.empty())
        {
            it = text.end();
            continue;
...
static void find_all_sourceforge(StringView text, Json::Array& packages, StringView version_text)
{
    ...
    while (it != text.end())
    {
        auto sfg = extract_first_cmake_invocation_args(StringView{it, text.end()}, "kmpkg_from_sourceforge");
        if (sfg.empty())
        {
            it = text.end();
            continue;
...
Json::Object kmpkg::run_resource_heuristics(StringView contents, StringView version_text)
{
...
std::string kmpkg::create_spdx_sbom(const InstallPlanAction& action,
...
{
    Checks::check_exit(KMPKG_LINE_INFO, relative_paths.size() == hashes.size());
    Checks::check_exit(KMPKG_LINE_INFO, relative_package_paths.size() == package_hashes.size());
    ...
    doc.insert(JsonIdName, fmt::format("{}@{} {}", action.spec, cpgh.version, abi));
    {
        ...
        creators.push_back(Strings::concat("Tool: kmpkg-", KMPKG_BASE_VERSION_AS_STRING, '-', KMPKG_VERSION_AS_STRING));
    ...
    auto& packages = doc.insert(JsonIdPackages, Json::Array());
    {
        ...
        obj.insert(JsonIdComment, "This is the port (recipe) consumed by kmpkg.");
        {
    ...
    }
    {
        ...
        obj.insert(SpdxLicenseDeclared, SpdxNoAssertion);
        obj.insert(SpdxCopyrightText, SpdxNoAssertion);
        obj.insert(JsonIdComment, "This is a binary package built by kmpkg.");
    }
        ...
        [&files,
         &rels](View<Path> paths, View<std::string> file_hashes, StringLiteral ref_name, StringLiteral ref_type) {
            for (size_t i = 0; i < paths.size(); ++i)
            {
                ...
                if (path == FileKmpkgDotJson && ref_type == SpdxRefPort)
                {
                    auto& rel = rels.push_back(Json::Object());
...
std::string kmpkg::calculate_spdx_license(const InstallPlanAction& action)
{
    const auto& scfl = action.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO);
    ...
    else
    {
        ...
        {
            licenses.push_back({SpdxLicenseRefKmpkgNull.to_string(), false});
        }
...
Optional<std::string> kmpkg::read_spdx_license_text(StringView text, StringView origin)
{
