...
namespace kmpkg
{
    ...
    Optional<InstalledPackageView> StatusParagraphs::get_installed_package_view(const PackageSpec& spec) const
    {
        ...
        for (auto&& p : *this)
        {
                ...
                p->is_installed())
            {
                ...
                {
                    Checks::check_exit(KMPKG_LINE_INFO, ipv.core == nullptr);
                    ipv.core = p.get();
                }
            }
        }
    ...
    bool kmpkg::StatusParagraphs::is_installed(const PackageSpec& spec) const
    {
        auto it = find(spec);
        return it != end() && (*it)->is_installed();
    }

    bool kmpkg::StatusParagraphs::is_installed(const FeatureSpec& spec) const
    {
        auto it = find(spec);
        return it != end() && (*it)->is_installed();
    }
    ...
    StatusParagraphs::iterator StatusParagraphs::insert(std::unique_ptr<StatusParagraph> pgh)
    {
        if (pgh == nullptr)
        {
            Checks::unreachable(KMPKG_LINE_INFO, "Inserted null paragraph");
        }
                              ...
                              const ReadOnlyFilesystem& fs)
    {
        ...
        auto owns_files = fs.read_lines(installed.listfile_path(ipv.core->package)).value_or_exit(KMPKG_LINE_INFO);
        Json::Array owns;
