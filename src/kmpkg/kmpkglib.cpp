...
namespace kmpkg
{
    static StatusParagraphs load_current_database(const ReadOnlyFilesystem& fs, const Path& kmpkg_dir_status_file)
    {
        auto pghs = Paragraphs::get_paragraphs(fs, kmpkg_dir_status_file).value_or_exit(KMPKG_LINE_INFO);

        std::vector<std::unique_ptr<StatusParagraph>> status_pghs;
        status_pghs.reserve(pghs.size());
        for (auto&& p : pghs)
        {
            status_pghs.push_back(std::make_unique<StatusParagraph>(kmpkg_dir_status_file, std::move(p)));
        }
    ...
    {
        auto update_files = fs.get_regular_files_non_recursive(updates_dir, KMPKG_LINE_INFO);
        ...
        if (!update_files.empty())
        {
            for (auto&& file : update_files)
            {
                ...
                auto pghs = Paragraphs::get_paragraphs(fs, file).value_or_exit(KMPKG_LINE_INFO);
                for (auto&& p : pghs)
                {
    ...
    static void apply_database_updates_on_disk(const Filesystem& fs,
                                               ...
                                               StatusParagraphs& current_status_db)
    {
        auto update_files = apply_database_updates(fs, current_status_db, installed.kmpkg_dir_updates());
        if (!update_files.empty())
        {
            const auto status_file = installed.kmpkg_dir_status_file();
            const auto status_file_new = Path(status_file.parent_path()) / FileStatusNew;
            fs.write_contents(status_file_new, Strings::serialize(current_status_db), KMPKG_LINE_INFO);
            fs.rename(status_file_new, status_file, KMPKG_LINE_INFO);
            for (auto&& file : update_files)
            {
                fs.remove(file, KMPKG_LINE_INFO);
            }
        }
    }
    ...
    {
        const auto maybe_status_file = installed.kmpkg_dir_status_file();
        if (!fs.exists(maybe_status_file, IgnoreErrors{}))
        {
            // no status file, use empty db
            StatusParagraphs current_status_db;
            (void)apply_database_updates(fs, current_status_db, installed.kmpkg_dir_updates());
    ...
    {
        const auto updates_dir = installed.kmpkg_dir_updates();

        fs.create_directories(installed.root(), KMPKG_LINE_INFO);
        fs.create_directory(installed.kmpkg_dir(), KMPKG_LINE_INFO);
        fs.create_directory(installed.kmpkg_dir_info(), KMPKG_LINE_INFO);
        fs.create_directory(updates_dir, KMPKG_LINE_INFO);
    ...
    void write_update(const Filesystem& fs, const InstalledPaths& installed, const StatusParagraph& p)
    {
        ...
        const auto update_path = installed.kmpkg_dir_updates() / fmt::format("{:010}", my_update_id);

        fs.write_rename_contents(update_path, FileIncomplete, Strings::serialize(p), KMPKG_LINE_INFO);
    }
    ...
    std::vector<InstalledPackageView> get_installed_ports(const StatusParagraphs& status_db)
    {
        ...
        {
            Checks::msg_check_exit(KMPKG_LINE_INFO, ipv.second.core != nullptr, msgCorruptedDatabase);
        }
                                                                                   ...
                                                                                   const StatusParagraphs& status_db)
    {
        ...
        for (const std::unique_ptr<StatusParagraph>& pgh : status_db)
        {
            ...
            std::vector<std::string> installed_files_of_current_pgh =
                fs.read_lines(listfile_path).value_or_exit(KMPKG_LINE_INFO);
            ...
            if (upgrade_to_slash_terminated_sorted_format(installed_files_of_current_pgh))
            {
                if constexpr (AndUpdate)
                {
                    ...
                    fs.write_lines(updated_listfile_path, installed_files_of_current_pgh, KMPKG_LINE_INFO);
                    fs.rename(updated_listfile_path, listfile_path, KMPKG_LINE_INFO);
                }
            }
    ...
    {
        Checks::check_exit(KMPKG_LINE_INFO, length >= 3);
