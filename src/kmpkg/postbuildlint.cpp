#include <kmpkg/base/cofffilereader.h>
...
namespace kmpkg
{
    static constexpr StringLiteral debug_lib_relative_path = "debug" KMPKG_PREFERRED_SEPARATOR "lib";
    ...
    static constexpr StringLiteral debug_bin_relative_path = "debug" KMPKG_PREFERRED_SEPARATOR "bin";
...
#undef OUTDATED_V_NO_120
    ...
    static LintStatus check_for_files_in_debug_include_directory(const ReadOnlyFilesystem& fs,
                                                                 ...
                                                                 MessageSink& msg_sink)
    {
        const auto debug_include_dir = package_dir / "debug" KMPKG_PREFERRED_SEPARATOR "include";
    ...
    static LintStatus check_for_kmpkg_port_config_in_cmake_helper_port(const ReadOnlyFilesystem& fs,
                                                                       const Path& package_dir,
                                                                       StringView package_name,
                                                                       const Path& portfile_cmake,
                                                                       MessageSink& msg_sink)
    {
        if (!fs.exists(package_dir / FileShare / package_name / FileKmpkgPortConfig, IgnoreErrors{}))
        {
            msg_sink.println(Color::warning,
                             LocalizedString::from_raw(portfile_cmake)
                                 .append_raw(": ")
                                                      ...
                                                      MessageSink& msg_sink)
    {
        static constexpr StringLiteral deny_relative_dirs[] = {
            "cmake",
            "debug" KMPKG_PREFERRED_SEPARATOR "cmake",
            "lib" KMPKG_PREFERRED_SEPARATOR "cmake",
            "debug" KMPKG_PREFERRED_SEPARATOR "lib" KMPKG_PREFERRED_SEPARATOR "cmake",
        };
    ...
    static LintStatus check_lib_cmake_merge(const ReadOnlyFilesystem& fs,
                                            ...
                                            MessageSink& msg_sink)
    {
        if (fs.exists(package_dir / "lib" KMPKG_PREFERRED_SEPARATOR "cmake", IgnoreErrors{}) ||
            fs.exists(package_dir / "debug" KMPKG_PREFERRED_SEPARATOR "lib" KMPKG_PREFERRED_SEPARATOR "cmake",
                      IgnoreErrors{}))
        {
            msg_sink.println(Color::warning,
                             LocalizedString::from_raw(portfile_cmake)
                                               ...
                                               MessageSink& msg_sink)
    {
        ...
        {
            // only one src dir, try to explain the kmpkg_install_copyright line for the user to use
                ...
                msg_sink.println(
                    LocalizedString::from_raw(portfile_cmake)
                        ...
                        .append(msgPortBugMissingLicenseFixIt,
                                msg::value = fmt::format(FMT_COMPILE("kmpkg_install_copyright(FILE_LIST {})"),
                                                         fmt::join(args, " "))));
            }
                                       ...
                                       std::vector<FileAndArch>& binaries_with_invalid_architecture)
    {
        for (size_t i = 0; i < relative_libs.size(); ++i)
        {
            ...
            auto machine_types = maybe_lib_information.value_or_exit(KMPKG_LINE_INFO).machine_types;
            {
                                                                    ...
                                                                    MessageSink& msg_sink)
    {
        ...
        msg_sink.println(
            LocalizedString::from_raw(NotePrefix)
                ...
                .append_raw('\n')
                .append_raw("if(KMPKG_LIBRARY_LINKAGE STREQUAL \"static\")\n")
                .append_indent()
                                                          ...
                                                          MessageSink& msg_sink)
    {
        ...
        static constexpr StringLiteral share_dir = "share" KMPKG_PREFERRED_SEPARATOR "pkgconfig";
        static constexpr StringLiteral lib_dir = "lib" KMPKG_PREFERRED_SEPARATOR "pkgconfig";
        static constexpr StringLiteral debug_dir =
            "debug" KMPKG_PREFERRED_SEPARATOR "lib" KMPKG_PREFERRED_SEPARATOR "pkgconfig";
        ...
        for (const Path& path : relative_all_files)
        {
            ...
            const bool contains_libs = Util::any_of(
                fs.read_lines(package_dir / path).value_or_exit(KMPKG_LINE_INFO), [](const std::string& line) {
            ...
            const bool is_debug = Strings::starts_with(path, "debug" KMPKG_PREFERRED_SEPARATOR);
            if (is_debug)
            {
        ...
        if (!misplaced_pkgconfig_files.empty())
        {
            ...
            msg_sink.println(LocalizedString::from_raw("kmpkg_fixup_pkgconfig()"));
        ...
        std::map<LinkageAndBuildType, std::vector<FileAndLinkages>>& groups_of_invalid_crt)
    {
        for (size_t i = 0; i < relative_libs.size(); ++i)
        {
            ...
            auto&& lib_info = maybe_lib_info.value_or_exit(KMPKG_LINE_INFO);
    ...
    static bool file_contains_absolute_paths(const ReadOnlyFilesystem& fs,
                                             const Path& file,
                                             View<Strings::kmpkg_searcher> searcher_paths)
    {
                                                 ...
                                                 MessageSink& msg_sink)
    {
        ...
        const auto searcher_paths =
            Util::fmap(string_paths, [](std::string& s) { return Strings::kmpkg_searcher(s.begin(), s.end()); });
                                                            ...
                                                            MessageSink& msg_sink)
    {
        ...
        if (policies.is_enabled(BuildPolicy::CMAKE_HELPER_PORT))
        {
            ...
            error_count += check_for_kmpkg_port_config_in_cmake_helper_port(
        ...
        if (windows_target)
        {
            ...
            if (!policies.is_enabled(BuildPolicy::SKIP_ARCHITECTURE_CHECK))
            {
                check_lib_architecture(pre_build_info.target_architecture,
                                       relative_debug_libs,
                                       debug_lib_info.value_or_exit(KMPKG_LINE_INFO),
                                       binaries_with_invalid_architecture);
                check_lib_architecture(pre_build_info.target_architecture,
                                       relative_release_libs,
                                       release_lib_info.value_or_exit(KMPKG_LINE_INFO),
                                       binaries_with_invalid_architecture);
            }
