...
#include <kmpkg/commands.add-version.h>
...
namespace
{
    ...
    static void write_json_file(const Filesystem& fs, const Json::Object& obj, const Path& output_path)
    {
        ...
        fs.create_directories(output_path.parent_path(), KMPKG_LINE_INFO);
        fs.write_contents(new_path, Json::stringify(obj), KMPKG_LINE_INFO);
        fs.rename(new_path, output_path, KMPKG_LINE_INFO);
    }
    ...
    UpdateResult update_baseline_version(const Filesystem& fs,
                                         ...
                                         std::map<std::string, kmpkg::Version, std::less<>>& baseline_map,
                                         bool print_success)
    {
    ...
    UpdateResult update_version_db_file(const KmpkgPaths& paths,
                                        const std::string& port_name,
...
namespace kmpkg
{
    constexpr CommandMetadata CommandAddVersionMetadata{
        ...
        {msgCmdAddVersionExample1, "kmpkg x-add-version curl --overwrite-version"},
        Undocumented,
    ...
    void command_add_version_and_exit(const KmpkgCmdArguments& args, const KmpkgPaths& paths)
    {
        ...
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgAddVersionFileNotFound, msg::path = baseline_path);
        }
        ...
        std::vector<GitLSTreeEntry> port_git_trees =
            paths.get_builtin_ports_directory_trees(console_diagnostic_context).value_or_exit(KMPKG_LINE_INFO);
        ...
        auto baseline_map = kmpkg::get_builtin_baseline(paths).value_or_exit(KMPKG_LINE_INFO);
        ...
        for (auto&& port_git_tree_entry : port_git_trees)
        {
            ...
            if (!skip_formatting_check)
            {
                ...
                if (scfl->control_path.filename() == FileKmpkgDotJson)
                {
                    ...
                    {
                        std::string command_line = "kmpkg format-manifest ";
