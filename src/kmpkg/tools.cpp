...
namespace kmpkg
{
    ...
    StringLiteral to_string_literal(ToolOs os) noexcept
    {
        ...
        Checks::unreachable(KMPKG_LINE_INFO, "Unexpected ToolOs");
    }
    ...
    ExpectedL<std::vector<ToolDataEntry>> parse_tool_data(StringView contents, StringView origin)
    {
        return Json::parse_object(contents, origin)
            .then([&](Json::Object&& as_object) -> ExpectedL<std::vector<ToolDataEntry>> {
                ...
                return maybe_tool_data.value_or_exit(KMPKG_LINE_INFO);
            });
    }
    ...
    struct NinjaProvider : ToolProvider
    {
        ...
        virtual void add_system_paths(const ReadOnlyFilesystem&, std::vector<Path>& out_candidate_paths) const override
        {
            // This is where Ninja goes by default on Alpine: https://github.com/kumose/kmpkg/issues/21218
    ...
    struct Python3Provider : ToolProvider
    {
        ...
        {
            for (auto&& candidate : fs.get_directories_non_recursive(program_files_root, KMPKG_LINE_INFO))
            {
    ...
    struct ToolCacheImpl final : ToolCache
    {
        ...
        kmpkg::Cache<std::string, PathAndVersion> path_version_cache;
        kmpkg::Lazy<std::vector<ToolDataEntry>> m_tool_data_cache;
        ...
        Path download_tool(const ToolData& tool_data, MessageSink& status_sink) const
        {
            ...
            switch (hash_result.prognosis)
            {
                ...
                case HashPrognosis::FileNotFound:
                    ...
                    break;
                case HashPrognosis::OtherError: Checks::exit_fail(KMPKG_LINE_INFO);
            ...
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgExpectedPathToExist, msg::path = exe_path);
            }
        ...
        PathAndVersion get_path(const ToolProvider& tool, MessageSink& status_sink) const
        {
            const bool env_force_system_binaries =
                get_environment_variable(EnvironmentVariableKmpkgForceSystemBinaries).has_value();
            const bool env_force_download_binaries =
                get_environment_variable(EnvironmentVariableKmpkgForceDownloadedBinaries).has_value();
            ...
            {
                // If there is an entry for the tool in kmpkg-tools.json, use that version as the minimum
            ...
            if (consider_downloads)
            {
                ...
                if (auto tool_data = maybe_tool_data.get())
                {
                    ...
                    auto downloaded_version =
                        tool.get_version(*this, status_sink, downloaded_path).value_or_exit(KMPKG_LINE_INFO);
            ...
            if (env_force_system_binaries && download_available)
            {
                s.append_raw(' ').append(msgDownloadAvailable,
                                         msg::env_var =
                                             format_environment_variable(EnvironmentVariableKmpkgForceSystemBinaries));
            }
            if (consider_system)
            {
            ...
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, s);
        }
        ...
        const PathAndVersion& get_tool_pathversion(StringView tool, MessageSink& status_sink) const
        {
            return path_version_cache.get_lazy(tool, [&]() -> PathAndVersion {
                ...
                if (tool == Tools::TAR)
                {
                    return {find_system_tar(fs).value_or_exit(KMPKG_LINE_INFO), {}};
                }
                if (tool == Tools::CMAKE_SYSTEM)
                {
                    return {find_system_cmake(fs).value_or_exit(KMPKG_LINE_INFO), {}};
                }
        ...
        std::vector<ToolDataEntry> load_tool_data() const
        {
            return m_tool_data_cache.get_lazy([&]() {
                ...
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, maybe_tool_data.error());
            });
        }
    };
