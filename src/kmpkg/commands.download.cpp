...
namespace kmpkg
{
    ...
    static Optional<std::string> get_sha512_check(const ParsedArguments& parsed)
    {
        ...
        if (parsed.command_arguments.size() > 1)
        {
            ...
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgShaPassedAsArgAndOption);
            }
        ...
        if (Util::Sets::contains(parsed.switches, SwitchSkipSha512))
        {
            if (sha.has_value())
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgShaPassedWithConflict);
            }
        }
        else if (!sha.has_value())
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgMissingOption, msg::option = "sha512");
        }
        ...
        if (auto p = sha.get())
        {
            if (!is_sha512(*p))
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgImproperShaLength, msg::value = *p);
            }
        }
    ...
    {
        // Note that we must NOT make a KmpkgPaths because that will chdir
        ...
        if (Util::Sets::contains(parsed.switches, SwitchStore))
        {
            ...
            if (!hash)
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgStoreOptionMissingSha);
            }

            auto s = fs.status(file, KMPKG_LINE_INFO);
            if (s != FileType::regular)
            {
                msg::println_error(msgIrregularFile, msg::path = file);
                Checks::unreachable(KMPKG_LINE_INFO);
            }
            auto actual_hash = Hash::get_file_hash(fs, file, Hash::Algorithm::Sha512).value_or_exit(KMPKG_LINE_INFO);
