...
namespace kmpkg
{
...
#if defined(_WIN32)
    std::string get_device_id(const kmpkg::Filesystem&)
    {
        ...
        auto new_device_id = Strings::ascii_to_lowercase(kmpkg::generate_random_UUID());
    ...
    std::string get_device_id(const kmpkg::Filesystem& fs)
    {
         ...
         * - The value is stored in UTF-8 plain text
         */
        const auto maybe_home_path = kmpkg::get_platform_cache_root();
        if (!maybe_home_path)
        {
            return {};
        }
