    ...
    namespace
    {
        ...
        BCRYPT_ALG_HANDLE get_alg_handle(LPCWSTR algorithm_identifier) noexcept
        {
            ...
            if (!NT_SUCCESS(error))
            {
                Checks::msg_exit_with_error(
                    KMPKG_LINE_INFO, msgFailedToOpenAlgorithm, msg::value = Strings::to_utf8(algorithm_identifier));
            }
        ...
        struct BCryptHasher : Hasher
        {
            ...
            virtual void add_bytes(const void* start_, const void* end_) noexcept override
            {
                ...
                Checks::check_exit(KMPKG_LINE_INFO, end - start >= 0);
                ...
                Checks::check_exit(KMPKG_LINE_INFO, end - start <= max);
#endif

                const auto length = static_cast<unsigned long>(end - start);
                const NTSTATUS error_code = BCryptHashData(hash_handle, start, length, 0);
                Checks::check_exit(KMPKG_LINE_INFO, NT_SUCCESS(error_code), "Failed to process a chunk");
            }
            ...
            virtual void clear() noexcept override
            {
                ...
                Checks::check_exit(KMPKG_LINE_INFO, NT_SUCCESS(error_code), "Failed to initialize the hasher");
            }
            ...
            virtual std::string get_hash() override
            {
                ...
                {
                    Checks::unreachable(KMPKG_LINE_INFO, "hash buffer too small");
                }

                uchar hash[static_max_hash_size];
                const NTSTATUS error_code = BCryptFinishHash(hash_handle, hash, hash_size, 0);
                Checks::check_exit(KMPKG_LINE_INFO, NT_SUCCESS(error_code), "Failed to finalize the hash");
        ...
        private:
            unsigned long get_hash_buffer_size() const
            {
                ...
                const NTSTATUS error_code = BCryptGetProperty(alg_handle,
                                                              ...
                                                              &cb_data,
                                                              0);
                Checks::check_exit(KMPKG_LINE_INFO, NT_SUCCESS(error_code), "Failed to get hash length");
