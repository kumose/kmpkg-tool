...
// kmpkg's current minimum supported git version (2.7.4). You can get a version of git that old with docker:
//
...
namespace
{
    ...
    Command make_git_command(const kmpkg::Path& git_exe, GitRepoLocator locator, View<StringView> additional_args)
    {
    ...
    Optional<std::string> run_cmd_trim(kmpkg::DiagnosticContext& context,
    ...
    Optional<std::string> run_cmd_trim(kmpkg::DiagnosticContext& context, const Command& command)
    {
