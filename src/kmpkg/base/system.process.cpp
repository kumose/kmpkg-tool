    ...
    struct CtrlCStateMachine
    {
        ...
        void transition_from_spawn_process() noexcept
        {
            ...
            if (previous == INT_MIN + 1)
            {
                ...
                // Therefore, exit
                Checks::log_final_cleanup_and_exit(KMPKG_LINE_INFO, 1);
            }
            else if (previous < 0)
            {
                // Ctrl-C was hit while blocked on the child process
...
namespace kmpkg
{
    ...
    Path get_exe_path_of_current_process()
    {
        ...
        Checks::check_exit(KMPKG_LINE_INFO, result != -1, "Could not determine current executable path.");
        ...
        Checks::check_exit(KMPKG_LINE_INFO, rcode == 0, "Could not determine current executable path.");
        Checks::check_exit(KMPKG_LINE_INFO, len > 0, "Could not determine current executable path.");
        return Path(exePath, len - 1);
#elif defined(__OpenBSD__)
        int mib[4] = {CTL_KERN, KERN_PROC_ARGS, getpid(), KERN_PROC_ARGV};
        size_t argc = 0;
        auto argc_query = sysctl(mib, 4, nullptr, &argc, nullptr, 0);
        Checks::check_exit(KMPKG_LINE_INFO, argc_query == 0, "Could not determine current executable path.");
        Checks::check_exit(KMPKG_LINE_INFO, argc > 0, "Could not determine current executable path.");
        std::vector<char*> argv(argc);
        auto argv_query = sysctl(mib, 4, &argv[0], &argc, nullptr, 0);
        Checks::check_exit(KMPKG_LINE_INFO, argv_query == 0, "Could not determine current executable path.");
        char buf[PATH_MAX];
        char* exePath(realpath(argv[0], buf));
        Checks::check_exit(KMPKG_LINE_INFO, exePath != nullptr, "Could not determine current executable path.");
        return Path(exePath);
        ...
        Checks::check_exit(KMPKG_LINE_INFO, written != -1, "Could not determine current executable path.");
        ...
        return Path(buf, written);
#else
        Checks::check_exit(KMPKG_LINE_INFO, false, "Could not determine current executable path.");
#endif
    }
...
namespace kmpkg
{
    void get_parent_process_list(std::vector<std::string>& ret)
    {
        ...
        auto maybe_kmpkg_stat = try_get_process_stat_by_pid(getpid());
        if (auto kmpkg_stat = maybe_kmpkg_stat.get())
        {
            for (auto next_parent = kmpkg_stat->ppid; next_parent != 0;)
            {
                if (Util::Sets::contains(seen_pids, next_parent))
                {
                    // parent graph loops, for example if a parent terminates and the PID is reused by a child launch
    ...
    {
        const std::string& system_root_env = get_system_root().value_or_exit(KMPKG_LINE_INFO).native();
        const std::string& system32_env = get_system32().value_or_exit(KMPKG_LINE_INFO).native();
        std::string new_path;
        ...
        std::vector<std::string> env_strings = {
            ...
            "Z_KMPKG_POSTSCRIPT",
            "Z_KMPKG_UNDO",
            // Ensures that the escape hatch persists to recursive kmpkg invocations like x-download
            "KMPKG_KEEP_ENV_VARS",
            // Enables Xbox SDKs
            "GameDKLatest",
            "GRDKLatest",
        ...
        const Optional<std::string> keep_vars = get_environment_variable(EnvironmentVariableKmpkgKeepEnvVars);
        ...
        if (!prepend_to_path.empty())
        {
            env.add_entry(
                ...
                Strings::concat(prepend_to_path,
                                ':',
                                get_environment_variable(EnvironmentVariablePath).value_or_exit(KMPKG_LINE_INFO)));
        }
...
namespace
{
    ...
    struct ProcessInfo : PROCESS_INFORMATION
    {
        ...
        unsigned long wait()
        {
            ...
            Checks::check_exit(KMPKG_LINE_INFO, result != WAIT_FAILED, "WaitForSingleObject failed");
            DWORD exit_code = 0;
                                ...
                                STARTUPINFOEXW& startup_info) noexcept
    {
        ...
        Optional<std::wstring> working_directory_wide = working_directory.map([](const Path& wd) {
            // this only fails if we can't get the current working directory of kmpkg, and we assume that we have that,
            // so it's fine anyways
            return Strings::to_utf16(real_filesystem.absolute(wd, KMPKG_LINE_INFO));
        });
        ...
        KMPKG_MSVC_WARNING(suppress : 6335)
    ...
    struct ProcAttributeList
    {
        ...
        {
            Checks::check_exit(KMPKG_LINE_INFO, buffer.empty());
            SIZE_T size = 0;
            ...
            Checks::check_exit(KMPKG_LINE_INFO, size > 0);
            ASSUME(size > 0);
            buffer.resize(size);
    ...
    struct AnonymousPipe
    {
        ...
        {
            Checks::check_exit(KMPKG_LINE_INFO, read_pipe == INVALID_HANDLE_VALUE);
            Checks::check_exit(KMPKG_LINE_INFO, write_pipe == INVALID_HANDLE_VALUE);
    ...
    struct OverlappedOutputPipe
    {
        ...
        bool create(DiagnosticContext& context, std::int32_t debug_id)
        {
            ...
            std::wstring pipe_name{Strings::to_utf16(
                fmt::format(R"(\\.\pipe\local\kmpkg-to-stdin-A8B4F218-4DB1-4A3E-8E5B-C41F1633F627-{}-{})",
                            GetCurrentProcessId(),
                            debug_id))};
    ...
    void drain_apcs()
    {
        switch (SleepEx(0, TRUE))
        {
            ...
            case WAIT_IO_COMPLETION:
                ...
                break;
            default: kmpkg::Checks::unreachable(KMPKG_LINE_INFO); break;
        }
    }
    ...
    struct RedirectedProcessInfo
    {
        ...
        KMPKG_MSVC_WARNING(suppress : 6262) // function uses 32k of stack
            ...
            static const auto stdin_completion_routine =
                [](DWORD dwErrorCode, DWORD dwNumberOfBytesTransferred, LPOVERLAPPED pOverlapped) {
                    ...
                    switch (dwErrorCode)
                    {
                        case 0:
                            // OK, done
                            Checks::check_exit(KMPKG_LINE_INFO, dwNumberOfBytesTransferred == status->expected_write);
                            break;
    ...
    struct PosixSpawnFileActions
    {
        ...
        PosixSpawnFileActions() { Checks::check_exit(KMPKG_LINE_INFO, posix_spawn_file_actions_init(&actions) == 0); }

        ~PosixSpawnFileActions()
        {
            Checks::check_exit(KMPKG_LINE_INFO, posix_spawn_file_actions_destroy(&actions) == 0);
        }
    ...
    Environment cmd_execute_and_capture_environment(const Command& cmd, const Environment& env)
    {
        ...
        if (!maybe_rc_output)
        {
            Checks::msg_exit_with_error(
                KMPKG_LINE_INFO, msg::format(msgVcvarsRunFailed).append_raw("\n").append(maybe_rc_output.error()));
        }

        auto& rc_output = maybe_rc_output.value_or_exit(KMPKG_LINE_INFO);
        Debug::print(rc_output.output, "\n");
        if (rc_output.exit_code != 0)
        {
            Checks::msg_exit_with_error(
                KMPKG_LINE_INFO, msgVcvarsRunFailedExitCode, msg::exit_code = rc_output.exit_code);
        }

        auto it = Strings::search(rc_output.output, magic_string);
        const char* const last = rc_output.output.data() + rc_output.output.size();

        Checks::check_exit(KMPKG_LINE_INFO, it != last);
                                                                ...
                                                                uint32_t debug_id)
    {
        ...
        switch (settings.encoding)
        {
            ...
            case Encoding::Utf8WithNulls:
                ...
                break;
            default: kmpkg::Checks::unreachable(KMPKG_LINE_INFO);
        }
        ...
        for (;;)
        {
            ...
            switch (settings.encoding)
            {
                ...
                case Encoding::Utf8WithNulls:
                    ...
                    break;
                default: Checks::unreachable(KMPKG_LINE_INFO); break;
            }
