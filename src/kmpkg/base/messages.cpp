...
#define DECLARE_MSG_ARG(NAME, EXAMPLE)                                                                                 \
    template<>                                                                                                         \
    struct ArgExample<::kmpkg::msg::NAME##_t>                                                                          \
    {                                                                                                                  \
...
#define DECLARE_MSG_ARG(NAME, EXAMPLE) const StringLiteral kmpkg::msg::NAME##_t::name = #NAME;
#include <kmpkg/base/message-args.inc.h>
#undef DECLARE_MSG_ARG
...
#undef DECLARE_MESSAGE
    ...
    namespace msg
    {
        namespace
        {
            ...
            {
                if (index >= detail::number_of_messages) Checks::unreachable(KMPKG_LINE_INFO);
...
#define DECLARE_MESSAGE(NAME, ARGS, COMMENT, ...)                                                                      \
    const decltype(::kmpkg::msg::detail::make_message_base ARGS) msg##NAME{static_cast<size_t>(message_index::NAME)};
...
namespace kmpkg::msg
{
    ...
    void load_from_message_map(const MessageMapAndFile& map_and_file)
    {
        ...
        for (size_t i = 0; i < detail::number_of_messages; ++i)
        {
            ...
            {
                a[i] = p->string(KMPKG_LINE_INFO).to_string();
            }
            else
            {
