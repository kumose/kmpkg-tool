...
namespace kmpkg::Json
{
    ...
    {
        kmpkg::Checks::check_exit(li, is_boolean());
        return underlying_->boolean;
    }
    int64_t Value::integer(LineInfo li) const noexcept
    {
        kmpkg::Checks::check_exit(li, is_integer());
    ...
    {
        kmpkg::Checks::msg_check_exit(li, is_string(), msgJsonValueNotString);
    ...
    {
        kmpkg::Checks::msg_check_exit(li, is_array(), msgJsonValueNotArray);
    ...
    {
        kmpkg::Checks::msg_check_exit(li, is_object(), msgJsonValueNotObject);
    ...
    {
        kmpkg::Checks::check_exit(KMPKG_LINE_INFO, isfinite(d));
        Value val;
    ...
    Value Value::string(std::string&& s) noexcept
    {
        if (!Unicode::utf8_is_valid_string(s.data(), s.data() + s.size()))
        {
            ...
            kmpkg::Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgInvalidString);
        }
        Value val;
    ...
    Object& Array::push_back(Object&& obj) { return push_back(Value::object(std::move(obj))).object(KMPKG_LINE_INFO); }
    Array& Array::push_back(Array&& arr) { return push_back(Value::array(std::move(arr))).array(KMPKG_LINE_INFO); }
    ...
    {
        return insert_before(it, Value::object(std::move(obj))).object(KMPKG_LINE_INFO);
    }
    Array& Array::insert_before(iterator it, Array&& arr)
    {
        return insert_before(it, Value::array(std::move(arr))).array(KMPKG_LINE_INFO);
    }
    ...
    {
        return insert(key, Value::array(std::move(value))).array(KMPKG_LINE_INFO);
    }
    Array& Object::insert(StringView key, const Array& value)
    {
        return insert(key, Value::array(value)).array(KMPKG_LINE_INFO);
    }
    Object& Object::insert(StringView key, Object&& value)
    {
        return insert(key, Value::object(std::move(value))).object(KMPKG_LINE_INFO);
    }
    Object& Object::insert(StringView key, const Object& value)
    {
        return insert(key, Value::object(value)).object(KMPKG_LINE_INFO);
    }
    ...
    {
        return insert_or_replace(key, Value::array(std::move(value))).array(KMPKG_LINE_INFO);
    }
    Array& Object::insert_or_replace(StringView key, const Array& value)
    {
        return insert_or_replace(key, Value::array(value)).array(KMPKG_LINE_INFO);
    }
    Object& Object::insert_or_replace(StringView key, Object&& value)
    {
        return insert_or_replace(key, Value::object(std::move(value))).object(KMPKG_LINE_INFO);
    }
    Object& Object::insert_or_replace(StringView key, const Object& value)
    {
        return insert_or_replace(key, Value::object(value)).object(KMPKG_LINE_INFO);
    }
    ...
    namespace
    {
        struct Parser : private ParserBase
        {
            ...
            static unsigned char from_hex_digit(char32_t code_point) noexcept
            {
                ...
                {
                    kmpkg::Checks::unreachable(KMPKG_LINE_INFO);
                }
            }
            ...
            {
                Checks::check_exit(KMPKG_LINE_INFO, cur() == '"');
                next();
            ...
            {
                Checks::check_exit(KMPKG_LINE_INFO, is_number_start(cur()));
            ...
            Value parse_object() noexcept
            {
                ...
                Checks::check_exit(KMPKG_LINE_INFO, current == '{');
                next();
        ...
        struct Stringifier
        {
            ...
            void stringify(const Value& value, size_t current_indent)
            {
                switch (value.kind())
                {
                    ...
                    case VK::Boolean:
                    {
                        auto v = value.boolean(KMPKG_LINE_INFO);
                        buffer.append(v ? "true" : "false");
                        break;
                    }
                    // TODO: switch to `to_chars` once we are able to remove support for old compilers
                    case VK::Integer: buffer.append(std::to_string(value.integer(KMPKG_LINE_INFO))); break;
                    case VK::Number: buffer.append(std::to_string(value.number(KMPKG_LINE_INFO))); break;
                    case VK::String:
                    {
                        append_quoted_json_string(value.string(KMPKG_LINE_INFO));
                        break;
                    }
                    case VK::Array:
                    {
                        stringify_array(value.array(KMPKG_LINE_INFO), current_indent);
                        break;
                    }
                    case VK::Object:
                    {
                        stringify_object(value.object(KMPKG_LINE_INFO), current_indent);
                        break;
                    }
                }
            }
