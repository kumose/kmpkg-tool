...
namespace
{
    ...
    std::string switch_name_to_display(kmpkg::StringView switch_name, StabilityTag stability)
    {
        std::string result{"--"};
    ...
    std::string switch_name_to_option_name(kmpkg::StringView switch_name)
    {
    ...
    bool try_parse_switch(kmpkg::StringView target, kmpkg::StringView switch_name, StabilityTag stability, bool& value)
    {
    ...
    TryParseOptionResult try_parse_option(kmpkg::StringView target_lowercase,
                                          kmpkg::StringView target,
                                          kmpkg::StringView option_name,
...
namespace kmpkg
{
    ...
    {
        Checks::check_exit(KMPKG_LINE_INFO, stability != StabilityTag::ImplementationDetail);
    ...
    bool CmdParser::parse_option(StringView option_name, StabilityTag stability, std::string& value)
    {
        ...
        for (std::size_t idx = 0; idx < argument_strings.size(); ++idx)
        {
            ...
            {
                Checks::check_exit(KMPKG_LINE_INFO, parse_result == TryParseOptionResult::ValueSet);
                ++found;
                argument_parsed[idx] = true;
            }
        }
    ...
    {
        Checks::check_exit(KMPKG_LINE_INFO, arity != 0); // use enforce_no_remaining_args instead
        Checks::check_exit(KMPKG_LINE_INFO, arity != 1); // use consume_only_remaining_arg instead
        std::vector<std::string> results;
    ...
    {
        Checks::check_exit(KMPKG_LINE_INFO, min_arity < max_arity); // if == use single parameter overload instead
        Checks::check_exit(KMPKG_LINE_INFO, max_arity != 0);        // use enforce_no_remaining_args instead
        Checks::check_exit(KMPKG_LINE_INFO, max_arity != 1);        // use consume_only_remaining_arg instead
        std::vector<std::string> results;
        bool error = consume_remaining_args_impl(results);
    ...
    void CmdParser::exit_with_errors(LocalizedString example)
    {
        ...
        Checks::exit_with_code(KMPKG_LINE_INFO, 1);
    }
}
