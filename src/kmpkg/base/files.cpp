...
namespace
{
    ...
    std::vector<Path> calculate_path_bases()
    {
        auto path_base_strings =
            Strings::split_paths(get_environment_variable(EnvironmentVariablePath).value_or_exit(KMPKG_LINE_INFO));
    ...
    struct FileHandle
    {
        ...
        KMPKG_MSVC_WARNING(push)
        KMPKG_MSVC_WARNING(disable : 6001)
        ~FileHandle()
        {
            if (h_file != INVALID_HANDLE_VALUE)
            {
                Checks::check_exit(KMPKG_LINE_INFO, ::CloseHandle(h_file));
            }
        }
        KMPKG_MSVC_WARNING(pop)
    };
    ...
    void kmpkg_remove_all_impl(const stdfs::directory_entry& current_entry, RemoveAllErrorInfo& err)
    {
        ...
        if (stdfs::is_directory(path_status))
        {
            ...
            for (stdfs::directory_iterator first(current_entry_path, ec);; first.increment(ec))
            {
                ...
                kmpkg_remove_all_impl(*first, err);
                if (err.check_ec(ec, current_entry_path))
                {
                    return;
                }
    ...
    void kmpkg_remove_all(const stdfs::directory_entry& current_entry, std::error_code& ec, Path& failure_point)
    {
        ...
        for (int backoff = 0; backoff < 5; ++backoff)
        {
            ...
            kmpkg_remove_all_impl(current_entry, err);
            if (!err.ec)
            {
                break;
            }
    ...
    void kmpkg_remove_all(const Path& base, std::error_code& ec, Path& failure_point)
    {
        ...
        kmpkg_remove_all(entry, ec, failure_point);
    }
    ...
    struct ReadDirOp
    {
        ...
        ~ReadDirOp()
        {
            if (dirp)
            {
                Checks::check_exit(KMPKG_LINE_INFO, closedir(dirp) == 0);
            }
        }
    };
    ...
    void kmpkg_remove_all_directory(const Path& base,
                                    std::error_code& ec,
                                    Path& failure_point,
                                    struct stat& base_lstat);

    void kmpkg_remove_all(const Path& base,
                          std::error_code& ec,
                          Path& failure_point,
                          PosixDType base_dtype = PosixDType::Unknown)
    {
        ...
        if (base_dtype == PosixDType::Unknown)
        {
            ...
            {
                kmpkg_remove_all_directory(base, ec, failure_point, s);
                return;
            }
        }
        else if (base_dtype == PosixDType::Directory)
        {
            // establish `base` being writable before calling kmpkg_remove_all_directory
            if (::lstat(base.c_str(), &s) != 0)
            {
    ...
    void kmpkg_remove_all_directory(const Path& base, std::error_code& ec, Path& failure_point, struct stat& base_lstat)
    {
        ...
        for (;;)
        {
            ...
            kmpkg_remove_all(base / entry->d_name, ec, failure_point, get_d_type(entry));
            if (ec)
            {
    ...
    constexpr char preferred_separator = KMPKG_PREFERRED_SEPARATOR[0];
} // unnamed namespace
...
namespace kmpkg
{
    ...
    Path Path::lexically_normal() const
    {
        ...
        // relicensed under MIT for the kmpkg repository.
    ...
    void FilePointer::close() noexcept
    {
        if (m_fs)
        {
            Checks::check_exit(KMPKG_LINE_INFO, ::fclose(m_fs) == 0);
            m_fs = 0;
        }
    }
    ...
    std::string ReadFilePointer::read_to_end(std::error_code& ec)
    {
        ...
        if (this_read != buffer_size)
        {
            ...
            if (!this->eof())
            {
                Checks::unreachable(KMPKG_LINE_INFO, "Got a partial read without an error or end");
            }
        }
    ...
    {
        return kmpkg::exists(this->symlink_status(target, ec));
    }
    ...
    FileType ReadOnlyFilesystem::status(const Path& target, kmpkg::LineInfo li) const noexcept
    {
        std::error_code ec;
    ...
    FileType ReadOnlyFilesystem::symlink_status(const Path& target, kmpkg::LineInfo li) const noexcept
    {
        std::error_code ec;
    ...
    int64_t Filesystem::last_write_time(const Path& target, kmpkg::LineInfo li) const noexcept
    {
        std::error_code ec;
    ...
    struct RealFilesystem final : Filesystem
    {
...
#if defined(_WIN32)
            result = kmpkg::win32_fix_path_case(result);
#endif // _WIN32
            return result;
        }
        ...
        virtual void write_lines(const Path& file_path,
                                 ...
                                 std::error_code& ec) const override
        {
            kmpkg::WriteFilePointer output{file_path, Append::NO, ec};
            if (!ec)
            {
        ...
        {
            kmpkg_remove_all(base, ec, failure_point);
        }
        ...
        virtual Path create_or_get_temp_directory(std::error_code& ec) const override
        {
            ...
            Path temp_folder_path = Path(Strings::to_utf8(temp_folder, length_without_null)) / "kmpkg";
#else  // ^^^ _WIN32 // !_WIN32 vvv
            const Path temp_folder_path =
                Path(get_environment_variable("TMPDIR").value_or(std::string("/tmp"))) / "kmpkg";
#endif // ^^^ !_WIN32
        ...
        struct ExclusiveFileLock : IExclusiveFileLock
        {
            ...
            {
                Checks::check_exit(KMPKG_LINE_INFO, handle == INVALID_HANDLE_VALUE);
            ...
            ~ExclusiveFileLock() override
            {
                if (handle != INVALID_HANDLE_VALUE)
                {
                    ...
                    Checks::check_exit(KMPKG_LINE_INFO, chresult != 0);
                }
            }
#else // ^^^ _WIN32 / !_WIN32 vvv
            PosixFd fd;
            ...
            ~ExclusiveFileLock() override
            {
                if (locked)
                {
                    Checks::check_exit(KMPKG_LINE_INFO, fd && fd.flock(LOCK_UN) == 0);
                }
            }
#endif
        };
    ...
    Path win32_fix_path_case(const Path& source)
    {
        ...
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgInvalidArgumentRequiresNoWildcards, msg::path = source);
        }
    ...
    void close_mark_invalid(int& fd) noexcept
    {
        if (fd >= 0)
        {
            Checks::check_exit(KMPKG_LINE_INFO, ::close(fd) == 0);
            fd = -1;
        }
    }
#endif // ^^^ !_WIN32
