...
#include <kmpkg/base/setup-messages.h>
...
namespace kmpkg
{
                                            ...
                                            const Optional<std::string>& script_sha512)
    {
        ...
        if (fs.exists(script_path, KMPKG_LINE_INFO))
        {
            return script_path;
        }
        ...
        if (auto sha = script_sha512.get())
        {
            ...
            url_prefix = "https://github.com/kumose/kmpkg-tool/releases/download/" KMPKG_BASE_VERSION_AS_STRING;
            filename.append(sha->data(), sha->size());
        }
        else
        {
            // not an official release, always use latest
            url_prefix = "https://github.com/kumose/kmpkg-tool/releases/latest/download";
            fmt::format_to(std::back_inserter(filename), "{}", kmpkg::get_process_id());
        }
        ...
        if (auto sha = script_sha512.get())
        {
            if (fs.exists(download_path, KMPKG_LINE_INFO))
            {
        ...
        fs.remove(download_path, KMPKG_LINE_INFO);
    ...
    {
        msg::println_warning(msgKmpkgCeIsExperimental);
        auto& fs = paths.get_filesystem();

        auto exe_path = get_exe_path_of_current_process();
        Optional<std::string> script_sha512;
#if defined(KMPKG_ARTIFACTS_SHA)
        script_sha512.emplace(MACRO_TO_STRING(KMPKG_ARTIFACTS_SHA));
#endif

        auto maybe_kmpkg_artifacts_path = provision_node_component(console_diagnostic_context,
                                                                   exe_path,
                                                                   ...
                                                                   "kmpkg-artifacts",
                                                                   script_sha512);
        auto kmpkg_artifacts_path = maybe_kmpkg_artifacts_path.get();
        if (!kmpkg_artifacts_path)
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgArtifactsBootstrapFailed);
        }

        auto temp_directory = fs.create_or_get_temp_directory(KMPKG_LINE_INFO);

        auto cmd = Command{paths.get_tool_exe(Tools::NODE, out_sink)};
        cmd.string_arg(*kmpkg_artifacts_path);
        ...
        cmd.string_arg("--kmpkg-root").string_arg(paths.root);
        cmd.string_arg("--z-kmpkg-command").string_arg(exe_path);

        cmd.string_arg("--z-kmpkg-artifacts-root").string_arg(paths.artifacts());
        cmd.string_arg("--z-kmpkg-downloads").string_arg(paths.downloads);
        cmd.string_arg("--z-kmpkg-registries-cache").string_arg(paths.registries_cache());
        ...
        if (!maybe_file.empty())
        {
            ...
            fs.write_contents(temp_file, maybe_file, KMPKG_LINE_INFO);
            cmd.string_arg("--language").string_arg(temp_file);
        }

        ProcessLaunchSettings settings;
        settings.working_directory = paths.original_cwd;
        const auto node_result = cmd_execute(cmd, settings).value_or_exit(KMPKG_LINE_INFO);
    ...
    void forward_common_artifacts_arguments(std::vector<std::string>& appended_to, const ParsedArguments& parsed)
    {
        ...
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgArtifactsSwitchOnlyOneOperatingSystem);
        }

        if (more_than_one_mapped(ArtifactHostPlatformSwitchNamesStorage, parsed.switches))
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgArtifactsSwitchOnlyOneHostPlatform);
        }

        if (more_than_one_mapped(ArtifactTargetPlatformSwitchNamesStorage, parsed.switches))
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgArtifactsSwitchOnlyOneTargetPlatform);
        }
