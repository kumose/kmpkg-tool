...
#include <kmpkg/commands.z-check-tools-sha.h>
#include <kmpkg/tools.test.h>
...
namespace kmpkg
{
    ...
    constexpr CommandMetadata CommandCheckToolsShaMetadata{
        ...
        {"kmpkg z-check-tools-sha scripts/kmpkg-tools.json"},
        Undocumented,
    ...
    void command_check_tools_sha_and_exit(const KmpkgCmdArguments& args, const Filesystem& fs)
    {
        const auto parsed = args.parse_arguments(CommandCheckToolsShaMetadata);

        const auto file_to_check = (fs.current_path(KMPKG_LINE_INFO) / parsed.command_arguments[0]).lexically_normal();

        const auto dest_path = fs.create_or_get_temp_directory(KMPKG_LINE_INFO);

        auto content = fs.read_contents(file_to_check, KMPKG_LINE_INFO);

        auto data = parse_tool_data(content, file_to_check).value_or_exit(KMPKG_LINE_INFO);
        ...
        for (auto& entry : data)
        {
            ...
            Checks::check_exit(KMPKG_LINE_INFO, iter.first->second == entry.sha512);
            if (iter.second)
            {
        ...
        for (auto& urlAndPath : urlAndPaths)
        {
            ...
            {
                auto sha =
                    Hash::get_file_hash(fs, urlAndPath.second, Hash::Algorithm::Sha512).value_or_exit(KMPKG_LINE_INFO);
            ...
            fs.remove(urlAndPath.second, KMPKG_LINE_INFO);
            ++http_codes_iter;
        }
        ...
        {
            Json::Object as_object = Json::parse_object(content, file_to_check).value_or_exit(KMPKG_LINE_INFO);
            int fixed = 0;
            for (auto&& entry : as_object["tools"].array(KMPKG_LINE_INFO))
            {
                auto maybe_url = entry.object(KMPKG_LINE_INFO).get("url");
                if (maybe_url)
                {
                    auto iter = url_to_fixed_sha.find(maybe_url->string(KMPKG_LINE_INFO).to_string());
                    if (iter != url_to_fixed_sha.end())
                    {
                        entry.object(KMPKG_LINE_INFO).insert_or_replace("sha512", iter->second);
                        ++fixed;
                    }
                }
            }

            fs.write_contents(file_to_check, Json::stringify(as_object), KMPKG_LINE_INFO);
            msg::println(msgFixedEntriesInFile, msg::count = fixed, msg::path = file_to_check);
            has_sha_error = false;
        }

        Checks::exit_with_code(KMPKG_LINE_INFO, (has_sha_error || has_http_error) ? EXIT_FAILURE : EXIT_SUCCESS);
    }
}
