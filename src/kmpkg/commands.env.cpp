...
namespace kmpkg
{
    constexpr CommandMetadata CommandEnvMetadata{
        ...
        msgHelpEnvCommand,
        {
            "kmpkg env --triplet x64-windows",
            msgCommandEnvExample2,
            "kmpkg env \"ninja --version\" --triplet x64-windows",
        },
        Undocumented,
    ...
    void command_env_and_exit(const KmpkgCmdArguments& args,
                              ...
                              Triplet /*host_triplet*/)
    {
        ...
        const PreBuildInfo pre_build_info(
            paths, triplet, var_provider.get_generic_triplet_vars(triplet).value_or_exit(KMPKG_LINE_INFO));
        ...
        if (add_tools)
        {
            ...
            for (auto&& tool_dir : fs.get_directories_non_recursive(tools_dir, KMPKG_LINE_INFO))
            {
        ...
        for (auto&& passthrough : pre_build_info.passthrough_env_vars)
        {
            ...
            {
                extra_env.emplace(passthrough, std::move(e.value_or_exit(KMPKG_LINE_INFO)));
            }
        }
        ...
        Checks::exit_with_code(KMPKG_LINE_INFO, static_cast<int>(rc.value_or_exit(KMPKG_LINE_INFO)));
#else  // ^^^ _WIN32 / !_WIN32 vvv
        Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgEnvPlatformNotSupported);
#endif // ^^^ !_WIN32
    }
} // namespace kmpkg
