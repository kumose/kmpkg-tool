...
namespace kmpkg
{
    ...
    static const char* parse_skip_number(const char* const s, uint64_t* const n)
    {
        ...
        *n = as_numeric({s, i}).value_or_exit(KMPKG_LINE_INFO);
        return s + i;
    }
    ...
    static VerComp compare_version_texts(VersionScheme sa, const Version& a, VersionScheme sb, const Version& b)
    {
        ...
        {
            return compare(DateVersion::try_parse(a.text).value_or_exit(KMPKG_LINE_INFO),
                           DateVersion::try_parse(b.text).value_or_exit(KMPKG_LINE_INFO));
        }

        if ((sa == VersionScheme::Semver || sa == VersionScheme::Relaxed) &&
            (sb == VersionScheme::Semver || sb == VersionScheme::Relaxed))
        {
            return compare(DotVersion::try_parse(a.text, sa).value_or_exit(KMPKG_LINE_INFO),
                           DotVersion::try_parse(b.text, sb).value_or_exit(KMPKG_LINE_INFO));
        }
