...
#include <kmpkg/base/parallel-algorithms.h>
...
#include <kmpkg/binarycaching.private.h>
...
namespace
{
    ...
    struct FilesWriteBinaryProvider : IWriteBinaryProvider
    {
        ...
        {
            const auto& zip_path = request.zip_path.value_or_exit(KMPKG_LINE_INFO);
            ...
            for (const auto& archives_root_dir : m_dirs)
            {
                ...
                if (!can_attempt_rename || (ec && ec == std::make_error_condition(std::errc::cross_device_link)))
                {
                    ...
                    // into place
                    // First copy to temporary location to avoid race between different kmpkg instances trying to upload
    ...
    struct ZipReadBinaryProvider : IReadBinaryProvider
    {
        ...
        void fetch(View<const InstallPlanAction*> actions, Span<RestoreResult> out_status) const override
        {
            ...
            for (size_t i = 0; i < actions.size(); ++i)
            {
                ...
                {
                    jobs.push_back({&actions[i]->package_dir.value_or_exit(KMPKG_LINE_INFO),
                                    zip_resource,
    ...
    struct FilesReadBinaryProvider : ZipReadBinaryProvider
    {
                          ...
                          Span<Optional<ZipResource>> out_zip_paths) const override
        {
            for (size_t i = 0; i < actions.size(); ++i)
            {
                const auto& abi_tag = actions[i]->package_abi_or_exit(KMPKG_LINE_INFO);
        ...
        void precheck(View<const InstallPlanAction*> actions, Span<CacheAvailability> cache_status) const override
        {
            for (size_t idx = 0; idx < actions.size(); ++idx)
            {
                ...
                const auto& abi_tag = action.package_abi_or_exit(KMPKG_LINE_INFO);
    ...
    struct AzureBlobPutBinaryProvider : IWriteBinaryProvider
    {
        ...
        size_t push_success(const BinaryPackageWriteInfo& request, MessageSink& msg_sink) override
        {
            ...
            size_t count_stored = 0;
            const auto file_size = m_fs.file_size(zip_path, KMPKG_LINE_INFO);
    ...
    struct NugetReadBinaryProvider : IReadBinaryProvider, private NugetBaseBinaryProvider
    {
        ...
        void fetch(View<const InstallPlanAction*> actions, Span<RestoreResult> out_status) const override
        {
            ...
            m_fs.write_contents(packages_config, generate_packages_config(refs), KMPKG_LINE_INFO);
            WarningDiagnosticContext wdc{console_diagnostic_context};
            ...
            for (size_t i = 0; i < actions.size(); ++i)
            {
                ...
                {
                    m_fs.remove(nupkg_path, KMPKG_LINE_INFO);
                    ...
                    if (nuget_dir != refs[i].id)
                    {
                        ...
                        m_fs.rename(path_from, path_to, KMPKG_LINE_INFO);
                    }
    ...
    struct NugetBinaryPushProvider : IWriteBinaryProvider, private NugetBaseBinaryProvider
    {
        ...
        size_t push_success(const BinaryPackageWriteInfo& request, MessageSink& msg_sink) override
        {
            ...
            auto& nuspec_contents = request.nuspec.value_or_exit(KMPKG_LINE_INFO);
            std::error_code ec;
    ...
    struct ObjectStorageProvider : ZipReadBinaryProvider
    {
                          ...
                          Span<Optional<ZipResource>> out_zip_paths) const override
        {
            ...
            {
                auto&& action = *actions[idx];
                const auto& abi = action.package_abi_or_exit(KMPKG_LINE_INFO);
    ...
    struct AzCopyStorageProvider : ZipReadBinaryProvider
    {
                          ...
                          Span<Optional<ZipResource>> out_zip_paths) const override
        {
            ...
            for (auto&& file : m_fs.get_files_non_recursive(tmp_downloads_location / container_name, KMPKG_LINE_INFO))
            {
    ...
    struct AzCopyStoragePushProvider : IWriteBinaryProvider
    {
        ...
        kmpkg::ExpectedL<Unit> upload_file(StringView url, const Path& archive) const
        {
    ...
    struct AzureUpkgPutBinaryProvider : public IWriteBinaryProvider
    {
        ...
        size_t push_success(const BinaryPackageWriteInfo& request, MessageSink& msg_sink) override
        {
            ...
            const Path& zip_path = request.zip_path.value_or_exit(KMPKG_LINE_INFO);
            for (auto&& write_src : m_sources)
            {
                auto res =
    ...
    struct AzureUpkgGetBinaryProvider : public ZipReadBinaryProvider
    {
        ...
        void acquire_zips(View<const InstallPlanAction*> actions, Span<Optional<ZipResource>> out_zips) const override
        {
            for (size_t i = 0; i < actions.size(); ++i)
            {
                ...
                {
                    m_fs.rename(temp_zip_path, final_zip_path, KMPKG_LINE_INFO);
                ...
                {
                    m_fs.remove(temp_dir, KMPKG_LINE_INFO);
                }
            }
        }
    ...
    {
        auto maybe_cachepath = get_environment_variable(EnvironmentVariableKmpkgDefaultBinaryCache);
        if (auto p_str = maybe_cachepath.get())
        {
            get_global_metrics_collector().track_define(DefineMetric::KmpkgDefaultBinaryCache);
        ...
        return get_platform_cache_kmpkg().then([](Path p) -> ExpectedL<Path> {
            if (p.is_absolute())
            {
                p /= "archives";
                p.make_preferred();
    ...
    private:
        ...
        {
            Checks::check_exit(KMPKG_LINE_INFO, !segments.empty());
...
namespace kmpkg
{
    ...
    static NuGetRepoInfo get_nuget_repo_info_from_env(const KmpkgCmdArguments& args)
    {
        if (auto p = args.kmpkg_nuget_repository.get())
        {
            get_global_metrics_collector().track_define(DefineMetric::KmpkgNuGetRepository);
            return {*p};
        }
    ...
    std::vector<CacheAvailability> ReadOnlyBinaryCache::precheck(View<const InstallPlanAction*> actions)
    {
        const std::vector<CacheStatus*> statuses = Util::fmap(actions, [this](const InstallPlanAction* action) {
            Checks::check_exit(KMPKG_LINE_INFO, action);
            return &m_status[action->package_abi_or_exit(KMPKG_LINE_INFO)];
        });
    ...
    void BinaryCacheSynchronizer::add_submitted() noexcept
    {
        ...
        {
            Checks::unreachable(KMPKG_LINE_INFO, "Maximum job count exceeded");
        }
    }
    ...
    bool BinaryCache::install_providers(const KmpkgCmdArguments& args,
                                        ...
                                        MessageSink& status_sink)
    {
        if (args.binary_caching_enabled())
        {
            ...
            {
                get_global_metrics_collector().track_define(DefineMetric::KmpkgBinarySources);
            }
    ...
    void BinaryCache::push_success(CleanPackages clean_packages, const InstallPlanAction& action)
    {
        ...
        {
            m_fs.remove_all(action.package_dir.value_or_exit(KMPKG_LINE_INFO), KMPKG_LINE_INFO);
        }
    }
    ...
    void BinaryCache::push_thread_main()
    {
        ...
        while (m_actions_to_push.get_work(my_tasks))
        {
            for (auto& action_to_push : my_tasks)
            {
                ...
                {
                    m_fs.remove_all(action_to_push.request.package_dir, KMPKG_LINE_INFO);
                }
    ...
    BinaryPackageReadInfo::BinaryPackageReadInfo(const InstallPlanAction& action)
        : package_abi(action.package_abi_or_exit(KMPKG_LINE_INFO))
        , spec(action.spec)
        , display_name(action.display_name())
        , version(action.version())
        , package_dir(action.package_dir.value_or_exit(KMPKG_LINE_INFO))
    {
    }
}

ExpectedL<AssetCachingSettings> kmpkg::parse_download_configuration(const Optional<std::string>& arg)
{
    ...
    AssetSourcesState s;
    const auto source = format_environment_variable(EnvironmentVariableXKmpkgAssetSources);
...
ExpectedL<BinaryConfigParserState> kmpkg::parse_binary_provider_configs(const std::string& env_string,
                                                                        View<std::string> args)
{
    ...
    const auto source = format_environment_variable("KMPKG_BINARY_SOURCES");
...
std::string kmpkg::format_version_for_feedref(StringView version_text, StringView abi_tag)
{
    ...
    if (try_extract_external_date_version(parsed_version, version_text))
    {
        ...
        return fmt::format(
            "{}.{}.{}-kmpkg{}", parsed_version.major, parsed_version.minor, parsed_version.patch, abi_tag);
    }
    ...
    return Strings::concat("0.0.0-kmpkg", abi_tag);
}

std::string kmpkg::generate_nuspec(const Path& package_dir,
                                   ...
                                   const NuGetRepoInfo& rinfo)
{
    ...
    const auto& compiler_info = abi_info.compiler_info.value_or_exit(KMPKG_LINE_INFO);
    ...
    std::string description =
        Strings::concat("NOT FOR DIRECT USE. Automatically generated cache package.\n\n",
                        ...
                        abi_info.triplet_abi.value_or_exit(KMPKG_LINE_INFO),
                        "\nFeatures:",
    ...
    xml.start_complex_open_tag("packageType").text_attr("name", "kmpkg").finish_self_closing_complex_tag();
...
LocalizedString kmpkg::format_help_topic_asset_caching()
{
...
LocalizedString kmpkg::format_help_topic_binary_caching()
{
    HelpTableFormatter table;
...
std::string kmpkg::generate_nuget_packages_config(const ActionPlan& plan, StringView prefix)
{
    XmlSerializer xml;
    xml.emit_declaration().line_break();
    xml.open_tag("packages").line_break();
...
FeedReference kmpkg::make_nugetref(const InstallPlanAction& action, StringView prefix)
{
    return ::make_feedref(
        action.spec, action.version(), action.abi_info.value_or_exit(KMPKG_LINE_INFO).package_abi, prefix);
}

std::vector<std::vector<std::string>> kmpkg::batch_command_arguments_with_fixed_length(
