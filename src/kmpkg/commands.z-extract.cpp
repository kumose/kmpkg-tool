...
namespace kmpkg
{
    ...
    static void extract_and_strip(
        const Filesystem& fs, const KmpkgPaths& paths, StripSetting strip_setting, Path archive_path, Path output_dir)
    {
        auto temp_dir =
            extract_archive_to_temp_subdirectory(fs, paths.get_tool_cache(), null_sink, archive_path, output_dir);

        ExtractedArchive archive = {
            temp_dir, output_dir, fs.get_regular_files_recursive_lexically_proximate(temp_dir, KMPKG_LINE_INFO)};
        ...
        for (const auto& file : mapping)
        {
            ...
            {
                fs.create_directories(destination.parent_path(), KMPKG_LINE_INFO);
            }

            fs.rename(source, destination, KMPKG_LINE_INFO);
        }

        fs.remove_all(temp_dir, KMPKG_LINE_INFO);
    }

    void command_z_extract_and_exit(const KmpkgCmdArguments& args, const KmpkgPaths& paths)
    {
        ...
        auto strip_setting = get_strip_setting(parse_args.settings).value_or_exit(KMPKG_LINE_INFO);

        if (!fs.is_directory(destination_path))
        {
            fs.create_directories(destination_path, KMPKG_LINE_INFO);
        }
