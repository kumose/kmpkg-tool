...
namespace kmpkg
{
    ...
    void command_z_generate_default_message_map_and_exit(const KmpkgCmdArguments& args, const Filesystem& fs)
    {
        ...
        auto artifact_messages_content = fs.try_read_contents(path_to_artifact_messages).value_or_exit(KMPKG_LINE_INFO);
        ...
        Path filepath = fs.current_path(KMPKG_LINE_INFO) / parsed_args.command_arguments[0];
        fs.write_contents(filepath, stringified, KMPKG_LINE_INFO);
