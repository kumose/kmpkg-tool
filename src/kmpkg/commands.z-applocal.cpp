...
#include <kmpkg/commands.z-applocal.h>
...
namespace
{
                                      ...
                                      const Filesystem& fs)
    {
        ...
        return fs.open_for_write(entry->second, KMPKG_LINE_INFO);
    }
    ...
    struct MutantGuard
    {
        MutantGuard(StringView name)
        {
            ...
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgFailedToAcquireMutant, msg::path = name);
            }
        }
    ...
    struct AppLocalInvocation
    {
        AppLocalInvocation(const Filesystem& fs,
            ...
            , m_openni2_installed(m_fs.exists(m_installed / "bin/OpenNI2/openni2deploy.ps1", KMPKG_LINE_INFO))
            , m_azurekinectsdk_installed(
                  m_fs.exists(m_installed / "tools/azure-kinect-sensor-sdk/k4adeploy.ps1", KMPKG_LINE_INFO))
            , m_magnum_installed(m_fs.exists(m_installed / "bin/magnum/magnumdeploy.ps1", KMPKG_LINE_INFO) ||
                                 m_fs.exists(m_installed / "bin/magnum-d/magnumdeploy.ps1", KMPKG_LINE_INFO))
            , m_qt_installed(m_fs.exists(m_installed / "plugins/qtdeploy.ps1", KMPKG_LINE_INFO))
        {
        }
        ...
        void resolve(const Path& binary)
        {
            ...
            auto dll_file = m_fs.open_for_read(binary, KMPKG_LINE_INFO);
            const auto dll_metadata = kmpkg::try_read_dll_metadata_required(dll_file).value_or_exit(KMPKG_LINE_INFO);
            const auto imported_names =
                kmpkg::try_read_dll_imported_dll_names(dll_metadata, dll_file).value_or_exit(KMPKG_LINE_INFO);
            dll_file.close();
        ...
        void resolve_explicit(const Path& binary, const std::vector<std::string>& imported_names)
        {
            ...
            for (auto&& imported_name : imported_names)
            {
                ...
                if (m_fs.exists(installed_item_file_path, KMPKG_LINE_INFO))
                {
                ...
                else if (m_fs.exists(target_item_file_path, KMPKG_LINE_INFO))
                {
    ...
    private:
        ...
        void deployQt(const Path& target_binary_dir, const Path& qt_plugins_dir, StringView target_binary_name)
        {
            ...
            else if (target_binary_name == "Qt5Qmld.dll" || target_binary_name == "Qt5Qml.dll")
            {
                ...
                if (!m_fs.exists(target_binary_dir / "qml", ec))
                {
                    ...
                    {
                        m_fs.copy_regular_recursive(bin_dir / "../qml", target_binary_dir, KMPKG_LINE_INFO);
                    }
        ...
        bool deploy_binary(const Path& target_binary_dir, const Path& installed_dir, StringView target_binary_name)
        {
            ...
            const auto mutant_name = "kmpkg-applocal-" + Hash::get_string_sha256(target_binary_dir);
            ...
            if (m_tlog_file)
            {
                ...
                Checks::check_exit(KMPKG_LINE_INFO, m_tlog_file.write(&native_newline, sizeof(wchar_t), 1) == 1);
            }
            ...
            if (m_copied_files_log)
            {
                ...
                Checks::check_exit(KMPKG_LINE_INFO, m_copied_files_log.put('\n') == '\n');
            }
...
namespace kmpkg
{
    constexpr CommandMetadata CommandZApplocalMetadata{
        ...
        {"kmpkg z-applocal --target-binary=\"Path/to/binary\" --installed-bin-dir=\"Path/to/installed/bin\" "
    ...
    void command_z_applocal_and_exit(const KmpkgCmdArguments& args, const Filesystem& fs)
    {
        ...
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgOptionRequiresAValue, msg::option = SwitchTargetBinary);
        }
        ...
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgOptionRequiresAValue, msg::option = SwitchInstalledBinDir);
        }

        const auto target_installed_bin_dir =
            fs.almost_canonical(target_installed_bin_setting->second, KMPKG_LINE_INFO);
        ...
        auto maybe_dll_metadata = kmpkg::try_read_dll_metadata(dll_file).value_or_exit(KMPKG_LINE_INFO);
