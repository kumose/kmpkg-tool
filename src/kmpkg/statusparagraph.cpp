...
namespace kmpkg
{
    ...
    StatusParagraph::StatusParagraph(StringView origin, Paragraph&& fields)
    {
        ...
        Checks::msg_check_exit(KMPKG_LINE_INFO, status_it != fields.end(), msgExpectedStatusField);
        auto status_field = std::move(status_it->second);
        fields.erase(status_it);
        this->package = BinaryParagraph(origin, std::move(fields));
        this->status =
            parse_status_line(status_field.first, origin, status_field.second).value_or_exit(KMPKG_LINE_INFO);
    }
