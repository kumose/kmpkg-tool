...
namespace kmpkg
{
    namespace
    {
        ...
        struct Cluster
        {
                             ...
                             Triplet host_triplet)
            {
                ...
                ClusterInstallInfo& info = m_install_info.value_or_exit(KMPKG_LINE_INFO);
                    ...
                    if (!info.defaults_requested)
                    {
                                         ...
                                         [](const auto& feature) { return !feature.platform.is_empty(); }))
                        {
                            if (auto maybe_vars = var_provider.get_dep_info_vars(m_spec))
                            {
                                ...
                                {
                                    if (f.platform.evaluate(maybe_vars.value_or_exit(KMPKG_LINE_INFO)))
                                    {
                ...
                const std::vector<Dependency>* qualified_deps = &maybe_qualified_deps.value_or_exit(KMPKG_LINE_INFO);
                        ...
                        if (dep.platform.evaluate(*vars))
                        {
                            ...
                            if (auto opt = dep.constraint.try_get_minimum_version())
                            {
                                info.version_constraints[fullspec.package_spec].insert(
                                    std::move(opt).value_or_exit(KMPKG_LINE_INFO));
                            }
                        }
                    }
            ...
            void create_install_info(std::vector<FeatureSpec>& out_reinstall_requirements)
            {
                ...
                Checks::check_exit(KMPKG_LINE_INFO, !m_install_info.has_value());
    ...
    const std::string& InstallPlanAction::public_abi() const
    {
        switch (plan_type)
        {
            case InstallPlanType::ALREADY_INSTALLED:
                return installed_package.value_or_exit(KMPKG_LINE_INFO).core->package.abi;
            case InstallPlanType::BUILD_AND_INSTALL:
            {
                auto&& i = abi_info.value_or_exit(KMPKG_LINE_INFO);
                                          ...
                                          std::vector<FeatureSpec>& out_reinstall_requirements) const
    {
        ...
        while (!to_remove.empty())
        {
            ...
            ClusterInstalled& info = clust.m_installed.value_or_exit(KMPKG_LINE_INFO);
                ...
                else
                {
                    ...
                    if (supports_expression && !supports_expression->is_empty())
                    {
                        if (!supports_expression->evaluate(
                                m_var_provider.get_dep_info_vars(spec.spec()).value_or_exit(KMPKG_LINE_INFO)))
                        {
                                       ...
                                       Editable editable_if_user_requested) const
    {
        struct BaseEdgeProvider : AdjacencyProvider<PackageSpec, const Cluster*>
        {
            ...
            {
                return &m_parent.find_or_exit(spec, KMPKG_LINE_INFO);
            }
        ...
        struct RemoveEdgeProvider final : BaseEdgeProvider
        {
            ...
            {
                auto&& set = vertex->m_installed.value_or_exit(KMPKG_LINE_INFO).remove_edges;
        ...
        struct InstallEdgeProvider final : BaseEdgeProvider
        {
            ...
            std::vector<PackageSpec> adjacency_list(const Cluster* const& vertex) const override
            {
                ...
                auto& info = vertex->m_install_info.value_or_exit(KMPKG_LINE_INFO);
        ...
        for (const auto* p_cluster : insert_toposort)
        {
            ...
            {
                auto&& installed = p_cluster->m_installed.value_or_exit(KMPKG_LINE_INFO);
    ...
    namespace
    {
        ...
        /**
         * kmpkg's Versioned Constraint Resolution Algorithm
         * ---
         *
         * Phase 1:
        ...
        const CMakeVars::CMakeVars& VersionedPackageGraph::batch_load_vars(const PackageSpec& spec)
        {
            ...
            if (!vars)
            {
                ...
                return m_var_provider.get_dep_info_vars(spec).value_or_exit(KMPKG_LINE_INFO);
            }
            return *vars.get();
        }
            ...
            Editable editable_if_user_requested)
        {
            ...
            auto push = [&emitted, this, &stack, use_head_version_if_user_requested, editable_if_user_requested](
                ...
                // logged to m_errors
                const auto& node = find_package(dep.spec).value_or_exit(KMPKG_LINE_INFO);
                    ...
                    return msg::format_error(msgCycleDetectedDuring, msg::spec = dep.spec)
                        ...
                        .append_raw(Strings::join("\n", stack, [](const Frame& p) {
                            return Strings::concat(
                                p.ipa.spec,
                                '@',
                                p.ipa.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO).to_version());
                        }));
                }
                return Unit{};
            };
                ...
                while (!stack.empty())
                {
                    ...
                    else
                    {
                        ...
                        const auto origin = Strings::concat(
                            back.ipa.spec,
                            "@",
                            back.ipa.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO).to_version());
                        x = push(dep, origin);
                        if (!x.has_value())
                        {
                            return std::move(x).error();
            ...
            for (auto&& action : ret.install_actions)
            {
                ...
                for (auto&& fdeps : action.feature_dependencies)
                {
                    ...
                    auto& fpgh = scfl.source_control_file->find_feature(fdeps.first).value_or_exit(KMPKG_LINE_INFO);
