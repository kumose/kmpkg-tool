...
namespace
{
    ...
    Path determine_root(const ReadOnlyFilesystem& fs, const Path& original_cwd, const KmpkgCmdArguments& args)
    {
        ...
        else
        {
            ...
            ret = fs.find_file_recursively_up(original_cwd, ".kmpkg-root", KMPKG_LINE_INFO);
            if (ret.empty())
            {
                ret = fs.find_file_recursively_up(canonical_current_exe, ".kmpkg-root", KMPKG_LINE_INFO);
            }
...
namespace kmpkg
{
    ...
    struct KmpkgPathsImpl : KmpkgPathsImplStage1
    {
        KmpkgPathsImpl(const Filesystem& fs,
                       ...
                       const Path& root,
                       const Path& original_cwd)
            : KmpkgPathsImplStage1(fs, args, bundle, root, original_cwd)
            ...
            , m_artifacts_dir(downloads / "artifacts")
        {
            ...
            if (!m_manifest_dir.empty())
            {
                ...
                {
                    std::error_code ec;
                    const auto kmpkg_root_file = root / ".kmpkg-root";
    ...
    KmpkgPaths::KmpkgPaths(const Filesystem& filesystem, const KmpkgCmdArguments& args, const BundleSettings& bundle)
        ...
        , m_pimpl(std::make_unique<KmpkgPathsImpl>(filesystem, args, bundle, root, original_cwd))
