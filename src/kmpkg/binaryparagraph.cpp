...
namespace kmpkg
{
        ...
        : spec(), version(), description(), maintainers(), feature(), default_features(), dependencies(), abi()
    {
        ...
        if (auto depends_field = maybe_depends_field.get())
        {
            this->dependencies = Util::fmap(
                ...
                [my_triplet](const ParsedQualifiedSpecifier& dep) {
                    // for compatibility with previous kmpkg versions, we discard all irrelevant information
                    return PackageSpec{
                        dep.name.value,
                        dep.triplet
        ...
        Checks::msg_check_exit(KMPKG_LINE_INFO, multi_arch == "same", msgMultiArch, msg::option = multi_arch);
    ...
    void serialize(const BinaryParagraph& pgh, std::string& out_str)
    {
        ...
        static constexpr StringLiteral sanity_parse_origin = "kmpkg::serialize(const BinaryParagraph&, std::string&)";
