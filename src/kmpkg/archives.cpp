...
namespace
{
    ...
    void win32_extract_nupkg(const ToolCache& tools, MessageSink& status_sink, const Path& archive, const Path& to_path)
    {
        ...
        {
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgCouldNotDeduceNuGetIdAndVersion, msg::path = archive);
        }
    ...
    void win32_extract_msi(const Path& archive, const Path& to_path)
    {
        ...
        for (unsigned int i = 0;; ++i)
        {
            ...
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, flatten(maybe_code_and_output, "msiexec").error());
        }
    }
    ...
    void win32_extract_with_seven_zip(const Path& seven_zip, const Path& archive, const Path& to_path)
    {
        ...
        Checks::check_exit(KMPKG_LINE_INFO, !recursion_limiter_sevenzip);
...
namespace kmpkg
{
                                              ...
                                              const Path& to_path)
    {
        ...
        fs.remove_all(to_path_partial, KMPKG_LINE_INFO);
        fs.create_directories(to_path_partial, KMPKG_LINE_INFO);
    ...
    void win32_extract_self_extracting_7z(const Filesystem& fs, const Path& archive, const Path& to_path)
    {
        ...
        auto contents = fs.read_contents(archive, KMPKG_LINE_INFO);
        ...
        fs.write_contents(to_path, contents_sv.substr(pos), KMPKG_LINE_INFO);
    }
#endif
    ...
    {
        fs.remove(destination, KMPKG_LINE_INFO);
#if defined(_WIN32)
        RedirectedProcessLaunchSettings settings;
        settings.environment = get_clean_environment();
        auto& seven_zip_path = seven_zip.value_or_exit(KMPKG_LINE_INFO);
    ...
    Command ZipTool::decompress_zip_archive_cmd(const Path& dst, const Path& archive_path) const
    {
        Command cmd;
#if defined(_WIN32)
        cmd.string_arg(seven_zip.value_or_exit(KMPKG_LINE_INFO))
            .string_arg("x")
