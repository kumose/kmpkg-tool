...
namespace kmpkg
{
    constexpr CommandMetadata CommandSetInstalledMetadata = {
        ...
        {msgCmdSetInstalledExample1, "kmpkg x-set-installed zlib:x64-windows boost"},
        Undocumented,
                                                            ...
                                                            const ActionPlan& action_plan)
    {
        ...
        if (github_ref && github_sha && github_job && github_workflow && github_run_id)
        {
...

            {
                ...
                detector.insert(JsonIdName, Json::Value::string("kmpkg"));
                detector.insert(JsonIdUrl, Json::Value::string("https://github.com/kumose/kmpkg"));
            ...
            for (auto&& action : action_plan.install_actions)
            {
                ...
                map.emplace(spec, fmt::format("pkg:github/kmpkg/{}@{}", spec, scfl->source_control_file->to_version()));
            }

            Json::Object manifest;
            manifest.insert(JsonIdName, FileKmpkgDotJson);
            ...
            manifests.insert(JsonIdKmpkgDotJson, manifest);
    ...
    std::set<PackageSpec> adjust_action_plan_to_status_db(ActionPlan& action_plan, const StatusParagraphs& status_db)
    {
        ...
        {
            all_abis.insert(action.abi_info.value_or_exit(KMPKG_LINE_INFO).package_abi);
        }
                                           ...
                                           bool include_manifest_in_github_issue)
    {
        ...
        {
            // Note that this differs from the behavior of `kmpkg install` in that it will print usage information for
        ...
        if (manifest && installed_paths)
        {
            ...
            const auto json_file_path = installed_paths->kmpkg_dir() / FileManifestInfo;
            const auto json_contents = Json::stringify(manifest_info);
            fs.write_contents(json_file_path, json_contents, KMPKG_LINE_INFO);
        }
