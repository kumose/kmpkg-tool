...
namespace kmpkg
{
    constexpr CommandMetadata CommandCheckSupportMetadata{
        ...
        {msgCmdCheckSupportExample1, "kmpkg x-check-support zlib"},
        Undocumented,
                                        ...
                                        Triplet host_triplet)
    {
        ...
        for (const auto& user_spec : specs)
        {
            ...
            for (const auto& action : action_plan.install_actions)
            {
                ...
                const auto& supports_expression = action.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO)
                                                      .source_control_file->core_paragraph->supports_expression;

                PlatformExpression::Context context = cmake_vars->get_tag_vars(spec).value_or_exit(KMPKG_LINE_INFO);
