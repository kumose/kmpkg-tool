...
namespace
{
    ...
    void open_for_write(const Filesystem& fs, const ToWrite& data)
    {
        ...
        fs.write_contents(data.file_to_write, Json::stringify(res), KMPKG_LINE_INFO);
        if (!in_place)
        {
            fs.remove(original_path_string, KMPKG_LINE_INFO);
        }
    }
...
namespace kmpkg
{
    constexpr CommandMetadata CommandFormatManifestMetadata{
        ...
        {msgCmdFormatManifestExample1, "kmpkg format-manifest ports/zlib/kmpkg.json", "kmpkg format-manifest --all"},
        Undocumented,
    ...
    void command_format_manifest_and_exit(const KmpkgCmdArguments& args, const KmpkgPaths& paths)
    {
        ...
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgFailedToFormatMissingFile);
        }
            ...
            if (path.filename() == "CONTROL")
            {
                ...
                if (auto control = maybe_control.get())
                {
                    to_write.push_back(ToWrite{contents->content,
                                               std::move(*control),
                                               std::move(path),
                                               Path(path.parent_path()) / "kmpkg.json"});
                }
                else
                {
                    has_error = true;
        ...
        if (format_all)
        {
            for (const auto& dir : fs.get_directories_non_recursive(paths.builtin_ports_directory(), KMPKG_LINE_INFO))
            {
                auto maybe_manifest =
                    ...
                    if (original.filename() == "CONTROL")
                    {
                        if (convert_control)
                        {
                            to_write.push_back(ToWrite{maybe_manifest.on_disk_contents,
                                                       ...
                                                       original,
                                                       Path(original.parent_path()) / "kmpkg.json"});
                        }
                    }
                    else
                    {
