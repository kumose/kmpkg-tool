...
namespace kmpkg
{
    constexpr CommandMetadata CommandZChangelogMetadata{
        "z-changelog",
        "Generate github.com/kumose/kmpkg changelog",
        {},
        Undocumented,
    ...
    void command_z_changelog_and_exit(const KmpkgCmdArguments& args, const KmpkgPaths& paths)
    {
        ...
        auto total_port_count = paths.get_filesystem()
                                    .get_directories_non_recursive(paths.builtin_ports_directory(), KMPKG_LINE_INFO)
                                    .size();
        ...
        result.append("The following kmpkg-tool releases have occurred since the last registry release:\n");
        result.append("* \n");
        result.append("\n");
