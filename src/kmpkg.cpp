...
namespace
{
    ...
    void invalid_command(const KmpkgCmdArguments& args)
    {
        msg::write_unlocalized_text_to_stderr(
            Color::error,
            LocalizedString::from_raw(ErrorPrefix)
                .append(msgKmpkgInvalidCommand, msg::command_name = args.get_command())
                .append_raw('\n'));
    ...
    void inner(const Filesystem& fs, const KmpkgCmdArguments& args, const BundleSettings& bundle)
    {
        // track version on each invocation
        get_global_metrics_collector().track_string(StringMetric::KmpkgVersion, kmpkg_executable_version);
        ...
        const KmpkgPaths paths(fs, args, bundle);
        ...
        fs.current_path(paths.root, KMPKG_LINE_INFO);
        ...
        Triplet default_triplet = kmpkg::default_triplet(args, paths.get_triplet_db());
        Triplet host_triplet = kmpkg::default_host_triplet(args, paths.get_triplet_db());
...
#endif
{
...
#endif
    set_environment_variable(EnvironmentVariableKmpkgCommand, get_exe_path_of_current_process().generic_u8string());
    ...
    if (!get_environment_variable(EnvironmentVariableKmpkgForceSystemBinaries).has_value())
    {
        Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgForceSystemBinariesOnWeirdPlatforms);
    }
#endif

    KmpkgCmdArguments args = KmpkgCmdArguments::create_from_command_line(real_filesystem, argc, argv);
    if (const auto p = args.debug.get()) Debug::g_debugging = *p;
    args.imbue_from_environment();
    KmpkgCmdArguments::imbue_or_apply_process_recursion(args);
    ...
    bool to_enable_metrics = true;
    {
        ...
        disable_metrics_tag_file_path.replace_filename("kmpkg.disable-metrics");
        std::error_code ec;
        if (real_filesystem.exists(disable_metrics_tag_file_path, ec) || ec)
        {
            Debug::println("Disabling metrics because kmpkg.disable-metrics exists");
            to_enable_metrics = false;
        }
    }

    auto bundle_path = current_exe_path;
    bundle_path.replace_filename(FileKmpkgBundleDotJson);
    ...
    auto data_blob = LocalizedString::from_raw(ErrorPrefix)
                         .append(msgKmpkgHasCrashed)
                         .append_raw("\nVersion=")
                         .append_raw(kmpkg_executable_version)
                         .append_raw("\nEXCEPTION=")
                         .append_raw(exc_msg)
                         .append_raw("\nCMD=\n");
