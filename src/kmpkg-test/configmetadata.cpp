...
static Configuration parse_test_configuration(StringView text)
{
    ...
    auto object = Json::parse_object(text, origin).value_or_exit(KMPKG_LINE_INFO);
    ...
    return std::move(parsed_config_opt).value_or_exit(KMPKG_LINE_INFO);
}
...
static void check_string(const Json::Object& obj, StringView key, StringView expected)
{
    ...
    REQUIRE(value->string(KMPKG_LINE_INFO) == expected);
}

static void check_errors(const std::string& config_text, const std::string& expected_errors)
{
    static constexpr StringLiteral origin = "test";
    auto object = Json::parse_object(config_text, origin).value_or_exit(KMPKG_LINE_INFO);
...
TEST_CASE ("config registries only", "[ce-metadata]")
{
    ...
    "registries": [
        {
            "kind": "git",
            "repository": "https://github.com/northwindtraders/kmpkg-registry",
        ...
        {
            "kind": "artifact",
            "name": "kmpkg-artifacts",
            "location": "https://github.com/kumose/kmpkg-artifacts"
        },
        {
            "kind": "filesystem",
            "path": "path/to/registry",
        ...
        auto default_registry = config.default_reg.get()->serialize().object(KMPKG_LINE_INFO);
        ...
        auto serialized_git_registry = git_registry.serialize().object(KMPKG_LINE_INFO);
        check_string(serialized_git_registry, KIND, "git");
        check_string(serialized_git_registry, REPOSITORY, "https://github.com/northwindtraders/kmpkg-registry");
        ...
        auto serialized_fs_registry = fs_registry.serialize().object(KMPKG_LINE_INFO);
        ...
        auto serialized_art_registry = artifact_registry.serialize().object(KMPKG_LINE_INFO);
        check_string(serialized_art_registry, KIND, "artifact");
        check_string(serialized_art_registry, NAME, "kmpkg-artifacts");
        check_string(serialized_art_registry, LOCATION, "https://github.com/kumose/kmpkg-artifacts");
        REQUIRE(!artifact_registry.packages);

        REQUIRE(config.registries[3].packages);

        auto raw_obj = Json::parse_object(raw_config, "test").value_or_exit(KMPKG_LINE_INFO);
...
test: error: $.registries[0] (a registry): missing required field 'packages' (a package pattern array)
    ...
    "registries": [{
        "kind": "git",
        "no-repository": "https://github.com/kumose/kmpkg",
        "baseline": "abcdef",
        "reference": {},
        "packages": {}
    }]
...
test: error: $.registries[0].packages: mismatched type: expected a package pattern array
    ...
    "registries": [{
        "kind": "artifact",
        "no-location": "https://github.com/kumose/kmpkg",
...
TEST_CASE ("metadata strings", "[ce-metadata]")
{
    ...
    "error": "this is a valid error"
        ...
        auto raw_obj = Json::parse_object(valid_raw, "test").value_or_exit(KMPKG_LINE_INFO);
...
TEST_CASE ("metadata dictionaries", "[ce-metadata]")
{
    ...
    "requires": {
        ...
        const auto& requires_ = requires_val->object(KMPKG_LINE_INFO);
        ...
        const auto& settings = settings_val->object(KMPKG_LINE_INFO);
...
TEST_CASE ("metadata demands", "[ce-metadata]")
{
    ...
    "demands": {
        ...
        const auto& level0 = level0_val->object(KMPKG_LINE_INFO);
        CHECK(level0.size() == 1);
        ...
        const auto& level1 = level1_val->object(KMPKG_LINE_INFO);
        CHECK(level1.size() == 1);
        check_string(level1, CE_MESSAGE, "this is level 1");

        auto raw_obj = Json::parse_object(simple_raw, "test").value_or_exit(KMPKG_LINE_INFO);
...
TEST_CASE ("serialize configuration", "[ce-metadata]")
{
    ...
    "overlay-ports": [
        ...
        Test::check_json_eq(Json::parse_object(raw, "test").value_or_exit(KMPKG_LINE_INFO), config.serialize());
    }
...
test: error: $.overlay-triplets[0]: mismatched type: expected a triplet path
    ...
    "demands": {
        ...
        auto config = parse_test_configuration(raw);
        Test::check_json_eq(Json::parse_object(formatted, "test").value_or_exit(KMPKG_LINE_INFO), config.serialize());
    }
}
...
TEST_CASE ("config with ce metadata full example", "[ce-metadata]")
{
    ...
    "error": "this is an error",
    "settings": {
        "KMPKG_ROOT": "C:/Users/viromer/work/kmpkg",
        "KMPKG_TARGET_TRIPLET": "arm-windows"
    },
    "apply": {
        "key": "value",
    ...
    auto serialized_registry = registry.serialize().object(KMPKG_LINE_INFO);
    check_string(serialized_registry, KIND, "git");
    check_string(serialized_registry, REPOSITORY, "https://github.com/northwindtraders/kmpkg-registry");
    ...
    */
    ...
    check_string(settings, "KMPKG_ROOT", "C:/Users/viromer/work/kmpkg");
    check_string(settings, "KMPKG_TARGET_TRIPLET", "arm-windows");
    ...
    */
    ...
    const auto& apply = apply_val->object(KMPKG_LINE_INFO);
    ...
    const auto& apply_complex_key = apply_complex_key_val->object(KMPKG_LINE_INFO);
    ...
    */
    ...
    const auto& demand1 = demand1_val->object(KMPKG_LINE_INFO);
    ...
    REQUIRE(demand1.get("number")->number(KMPKG_LINE_INFO) == 2);
    ...
    const auto& demand1_requires = demand1_requires_val->object(KMPKG_LINE_INFO);
    ...
    */
    ...
    const auto& demand2 = demand2_val->object(KMPKG_LINE_INFO);
    ...
    const auto& demand2_error = demand2_error_val->object(KMPKG_LINE_INFO);
