...
TEST_CASE ("JSON parse keywords", "[json]")
{
    ...
    REQUIRE(res.get()->value.boolean(KMPKG_LINE_INFO));
...
TEST_CASE ("JSON parse strings", "[json]")
{
    ...
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO).size() == 0);

    res = Json::parse(R"("\ud800")", "test"); // unpaired surrogate
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == "\xED\xA0\x80");
    ...
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == grin.to_string());

    res = Json::parse(make_json_string(radical), "test"); // character in BMP
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == radical);

    res = Json::parse(make_json_string(grin), "test"); // character above BMP
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == grin);
}
...
TEST_CASE ("JSON parse strings with escapes", "[json]")
{
    ...
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == "\t");
    ...
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == "\b");

    res = Json::parse(R"("\f")", "test");
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == "\f");
    ...
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == "\r");

    res = Json::parse(R"("This is a \"test\", hopefully it worked")", "test");
    REQUIRE(res);
    REQUIRE(res.get()->value.is_string());
    REQUIRE(res.get()->value.string(KMPKG_LINE_INFO) == R"(This is a "test", hopefully it worked)");
}
...
TEST_CASE ("JSON parse integers", "[json]")
{
    ...
    REQUIRE(res.get()->value.integer(KMPKG_LINE_INFO) == 0);
    res = Json::parse("12345", "test");
    REQUIRE(res);
    REQUIRE(res.get()->value.is_integer());
    REQUIRE(res.get()->value.integer(KMPKG_LINE_INFO) == 12345);
    res = Json::parse("-12345", "test");
    REQUIRE(res);
    REQUIRE(res.get()->value.is_integer());
    REQUIRE(res.get()->value.integer(KMPKG_LINE_INFO) == -12345);
    ...
    REQUIRE(res);
    REQUIRE(res.get()->value.is_integer());
    REQUIRE(res.get()->value.integer(KMPKG_LINE_INFO) == 9223372036854775807);
    res = Json::parse("-9223372036854775808", "test");
    REQUIRE(res);
    REQUIRE(res.get()->value.is_integer());
    REQUIRE(res.get()->value.integer(KMPKG_LINE_INFO) == (-9223372036854775807 - 1)); // INT64_MIN (C++'s parser is fun)
}
...
TEST_CASE ("JSON parse floats", "[json]")
{
    ...
    REQUIRE(!res.get()->value.is_integer());
    REQUIRE(res.get()->value.number(KMPKG_LINE_INFO) == 0.0);
    REQUIRE(!signbit(res.get()->value.number(KMPKG_LINE_INFO)));
    ...
    REQUIRE(res);
    REQUIRE(res.get()->value.is_number());
    REQUIRE_THAT(res.get()->value.number(KMPKG_LINE_INFO), Catch::WithinULP(12345.6789, 3));
    res = Json::parse("-12345.6789", "test");
    REQUIRE(res);
    REQUIRE(res.get()->value.is_number());
    REQUIRE_THAT(res.get()->value.number(KMPKG_LINE_INFO), Catch::WithinULP(-12345.6789, 3));
...
TEST_CASE ("JSON parse arrays", "[json]")
{
    ...
    REQUIRE(val.array(KMPKG_LINE_INFO).size() == 0);

    res = Json::parse("[123]", "test");
    REQUIRE(res);
    val = std::move(res.get()->value);
    REQUIRE(val.is_array());
    REQUIRE(val.array(KMPKG_LINE_INFO).size() == 1);
    REQUIRE(val.array(KMPKG_LINE_INFO)[0].is_integer());
    REQUIRE(val.array(KMPKG_LINE_INFO)[0].integer(KMPKG_LINE_INFO) == 123);

    res = Json::parse("[123, 456]", "test");
    REQUIRE(res);
    val = std::move(res.get()->value);
    REQUIRE(val.is_array());
    REQUIRE(val.array(KMPKG_LINE_INFO).size() == 2);
    REQUIRE(val.array(KMPKG_LINE_INFO)[0].is_integer());
    REQUIRE(val.array(KMPKG_LINE_INFO)[0].integer(KMPKG_LINE_INFO) == 123);
    REQUIRE(val.array(KMPKG_LINE_INFO)[1].is_integer());
    REQUIRE(val.array(KMPKG_LINE_INFO)[1].integer(KMPKG_LINE_INFO) == 456);

    res = Json::parse("[123, 456, [null]]", "test");
    REQUIRE(res);
    val = std::move(res.get()->value);
    REQUIRE(val.is_array());
    REQUIRE(val.array(KMPKG_LINE_INFO).size() == 3);
    REQUIRE(val.array(KMPKG_LINE_INFO)[2].is_array());
    REQUIRE(val.array(KMPKG_LINE_INFO)[2].array(KMPKG_LINE_INFO).size() == 1);
    REQUIRE(val.array(KMPKG_LINE_INFO)[2].array(KMPKG_LINE_INFO)[0].is_null());
}
...
TEST_CASE ("JSON parse objects", "[json]")
{
    ...
    REQUIRE(val.object(KMPKG_LINE_INFO).size() == 0);
}
