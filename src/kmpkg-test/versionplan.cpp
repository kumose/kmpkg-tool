...
#include <kmpkg/paragraphparser.h>
...
TEST_CASE ("parse depends", "[dependencies]")
{
    ...
    REQUIRE(v.at(0).platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", ""}}));
    REQUIRE(v.at(0).platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", "WindowsStore"}}));
    REQUIRE(!v.at(0).platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", "Darwin"}}));
}
...
TEST_CASE ("filter depends", "[dependencies]")
{
    ...
    const std::unordered_map<std::string, std::string> x64_win_cmake_vars{{"KMPKG_TARGET_ARCHITECTURE", "x64"},
                                                                          {"KMPKG_CMAKE_SYSTEM_NAME", ""}};

    const std::unordered_map<std::string, std::string> arm_uwp_cmake_vars{{"KMPKG_TARGET_ARCHITECTURE", "arm"},
                                                                          {"KMPKG_CMAKE_SYSTEM_NAME", "WindowsStore"}};
...
TEST_CASE ("parse feature depends", "[dependencies]")
{
    ...
    REQUIRE(a1.platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", ""}}));
    REQUIRE(a1.platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", "Linux"}}));
    REQUIRE_FALSE(a1.platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", "Darwin"}}));
}
...
TEST_CASE ("qualified dependency", "[dependencies]")
{
    ...
    var_provider.dep_info_vars[{"a", Test::X64_LINUX}].emplace("KMPKG_CMAKE_SYSTEM_NAME", "Linux");
    ...
    auto plan = kmpkg::create_feature_install_plan(
    ...
    REQUIRE(plan.install_actions[0].package_dir == "pkg" KMPKG_PREFERRED_SEPARATOR "b_x86-windows");

    auto plan2 = kmpkg::create_feature_install_plan(
    ...
    REQUIRE(plan2.install_actions[0].package_dir == "pkg" KMPKG_PREFERRED_SEPARATOR "b_x64-linux");
}
