...
namespace
{
    ...
    void do_filesystem_enumeration_test(Enumerator&& enumerator, ExpectedGenerator&& generate_expected)
    {
        ...
        fs.remove_all(temp_dir, KMPKG_LINE_INFO);

        fs.create_directory(temp_dir, KMPKG_LINE_INFO);
        fs.create_directory(target_root, KMPKG_LINE_INFO);
        fs.create_directory(target_directory, KMPKG_LINE_INFO);
        fs.create_directory(target_inside_directory, KMPKG_LINE_INFO);

        fs.write_contents(target_file, "file", KMPKG_LINE_INFO);
        fs.write_contents(target_inside_file, "file in directory", KMPKG_LINE_INFO);
        ...
        {
            fs.create_symlink(target_inside_file, target_inside_symlink, KMPKG_LINE_INFO);
            fs.create_directory_symlink(target_directory, target_directory_symlink, KMPKG_LINE_INFO);
            fs.create_directory_symlink(target_inside_directory, target_inside_directory_symlink, KMPKG_LINE_INFO);
...
TEST_CASE ("kmpkg Path regular operations", "[filesystem][files]")
{
...
TEST_CASE ("kmpkg Path conversions", "[filesystem][files]")
{
...
TEST_CASE ("kmpkg Path generic", "[filesystem][files]")
{
...
TEST_CASE ("kmpkg Path::operator/", "[filesystem][files]")
{
    test_op_slash("/a/b", "c/d", "/a/b" KMPKG_PREFERRED_SEPARATOR "c/d");
    test_op_slash("a/b", "c/d", "a/b" KMPKG_PREFERRED_SEPARATOR "c/d");
...
TEST_CASE ("kmpkg Path::preferred and Path::make_preferred", "[filesystem][files]")
{
    test_preferred("", "");
    test_preferred("hello", "hello");
    test_preferred("/hello", KMPKG_PREFERRED_SEPARATOR "hello");
    test_preferred("hello/", "hello" KMPKG_PREFERRED_SEPARATOR);
    test_preferred("hello/////////there", "hello" KMPKG_PREFERRED_SEPARATOR "there");
    test_preferred("hello/////////there///" KMPKG_PREFERRED_SEPARATOR "world",
                   "hello" KMPKG_PREFERRED_SEPARATOR "there" KMPKG_PREFERRED_SEPARATOR "world");
    test_preferred("/a/b", KMPKG_PREFERRED_SEPARATOR "a" KMPKG_PREFERRED_SEPARATOR "b");
    test_preferred("a/b", "a" KMPKG_PREFERRED_SEPARATOR "b");
...
TEST_CASE ("remove readonly", "[files]")
{
    ...
    fs.create_directory(writable_dir, KMPKG_LINE_INFO);

    const auto writable_dir_writable_file = writable_dir / "writable_file";
    fs.write_contents(writable_dir_writable_file, "content", KMPKG_LINE_INFO);

    const auto writable_dir_readonly_file = writable_dir / "readonly_file";
    fs.write_contents(writable_dir_readonly_file, "content", KMPKG_LINE_INFO);
    set_readonly(writable_dir_readonly_file);

    CHECK(fs.remove(writable_dir_writable_file, KMPKG_LINE_INFO));
    CHECK(fs.remove(writable_dir_readonly_file, KMPKG_LINE_INFO));

    CHECK(fs.remove(writable_dir, KMPKG_LINE_INFO));
    ...
    // would matter, kmpkg doesn't create directories without writable bits (for now).
    const auto readonly_dir = temp_dir / "readonly_dir";
    fs.create_directory(readonly_dir, KMPKG_LINE_INFO);

    const auto readonly_dir_writable_file = readonly_dir / "writable_file";
    fs.write_contents(readonly_dir_writable_file, "content", KMPKG_LINE_INFO);

    const auto readonly_dir_readonly_file = readonly_dir / "readonly_file";
    fs.write_contents(readonly_dir_readonly_file, "content", KMPKG_LINE_INFO);
    set_readonly(readonly_dir_readonly_file);

    set_readonly(readonly_dir);

    CHECK(fs.remove(readonly_dir_writable_file, KMPKG_LINE_INFO));
    CHECK(fs.remove(readonly_dir_readonly_file, KMPKG_LINE_INFO));

    CHECK(fs.remove(readonly_dir, KMPKG_LINE_INFO));
#endif // ^^^ _WIN32

    CHECK(fs.remove(temp_dir, KMPKG_LINE_INFO));
    std::error_code ec;
...
TEST_CASE ("remove all symlinks", "[files]")
{
    ...
    const auto target_root = temp_dir / "target";
    fs.create_directories(target_root, KMPKG_LINE_INFO);
    const auto target_file = target_root / "file.txt";
    fs.write_contents(target_file, "", KMPKG_LINE_INFO);
    const auto symlink_inside_dir = temp_dir / "symlink_inside";
    fs.create_directory(symlink_inside_dir, KMPKG_LINE_INFO);
    std::error_code ec;
    ...
    else
    {
        ...
        fs.create_directory_symlink(target_root, symlink_direct, KMPKG_LINE_INFO);

        // removing a directory with a symlink inside should remove the symlink and not the target:
        fs.remove_all(symlink_inside_dir, KMPKG_LINE_INFO);
        REQUIRE(!fs.exists(symlink_inside_dir, KMPKG_LINE_INFO));
        REQUIRE(fs.exists(target_root, KMPKG_LINE_INFO));

        // removing a symlink should remove the symlink and not the target:
        fs.remove_all(symlink_direct, KMPKG_LINE_INFO);
        REQUIRE(!fs.exists(symlink_direct, KMPKG_LINE_INFO));
        REQUIRE(fs.exists(target_root, KMPKG_LINE_INFO));
    }
...
TEST_CASE ("get_files_recursive_symlinks", "[files]")
{
    do_filesystem_enumeration_test(
        [](const Filesystem& fs, const Path& root) { return fs.get_files_recursive(root, KMPKG_LINE_INFO); },
        [](const Path& root) {
...
TEST_CASE ("get_regular_files_recursive_proximate_symlinks", "[files]")
{
    do_filesystem_enumeration_test(
        [](const Filesystem& fs, const Path& root) {
            return fs.get_regular_files_recursive_lexically_proximate(root, KMPKG_LINE_INFO);
        },
        [](const Path&) {
...
{
    do_filesystem_enumeration_test(
        [](const Filesystem& fs, const Path& root) { return fs.get_files_non_recursive(root, KMPKG_LINE_INFO); },
        [](const Path& root) {
            return std::vector<Path>{
                root / "file.txt",
                root / "some-directory",
...
{
    do_filesystem_enumeration_test(
        [](const Filesystem& fs, const Path& root) { return fs.get_directories_recursive(root, KMPKG_LINE_INFO); },
        [](const Path& root) {
            return std::vector<Path>{
                root / "some-directory",
                root / "some-directory" / "some-inner-directory",
...
{
    do_filesystem_enumeration_test(
        [](const Filesystem& fs, const Path& root) { return fs.get_directories_non_recursive(root, KMPKG_LINE_INFO); },
        [](const Path& root) {
            return std::vector<Path>{
                root / "some-directory",
                root / "symlink-to-some-directory",
...
{
    do_filesystem_enumeration_test(
        [](const Filesystem& fs, const Path& root) { return fs.get_regular_files_recursive(root, KMPKG_LINE_INFO); },
        [](const Path& root) {
            return std::vector<Path>{
                root / "file.txt",
                root / "some-directory" / "file2.txt",
...
TEST_CASE ("get_regular_files_non_recursive_symlinks", "[files]")
{
    do_filesystem_enumeration_test(
        [](const Filesystem& fs, const Path& root) {
            return fs.get_regular_files_non_recursive(root, KMPKG_LINE_INFO);
        },
        [](const Path& root) {
            return std::vector<Path>{
                root / "file.txt",
...
TEST_CASE ("copy_file", "[files]")
{
    ...
    fs.write_contents(existing_from, existing_from_contents, KMPKG_LINE_INFO);

    const auto existing_to = temp_dir / "already_existing";
    constexpr StringLiteral existing_to_contents = "already existing file";
    fs.write_contents(existing_to, existing_to_contents, KMPKG_LINE_INFO);
    ...
    fs.create_directory(temp_dir / "a_directory", KMPKG_LINE_INFO);
    ...
    REQUIRE(!ec);
    REQUIRE(fs.read_contents(temp_dir / "b", KMPKG_LINE_INFO) == existing_from_contents);

    // [skip_existing]
    REQUIRE(!fs.copy_file(existing_from, existing_to, CopyOptions::skip_existing, ec));
    REQUIRE(!ec);
    REQUIRE(fs.read_contents(existing_to, KMPKG_LINE_INFO) == existing_to_contents);

    // [overwrite_existing]
    REQUIRE(fs.copy_file(existing_from, existing_to, CopyOptions::overwrite_existing, ec));
    REQUIRE(!ec);
    REQUIRE(fs.read_contents(existing_to, KMPKG_LINE_INFO) == existing_from_contents);
    ...
    fs.copy_file(existing_from, attributes_target, CopyOptions::none, KMPKG_LINE_INFO);
    REQUIRE(fs.read_contents(attributes_target, KMPKG_LINE_INFO) == existing_from_contents);
...
TEST_CASE ("rename", "[files]")
{
    ...
    fs.create_directory(temp_dir_a, KMPKG_LINE_INFO);
    ...
    fs.write_contents(temp_dir_a_file, text_file_contents, KMPKG_LINE_INFO);

    // try rename_with_retry
    {
        fs.rename_with_retry(temp_dir_a, temp_dir_b, KMPKG_LINE_INFO);
        REQUIRE(!fs.exists(temp_dir_a, KMPKG_LINE_INFO));
        REQUIRE(fs.read_contents(temp_dir_b_file, KMPKG_LINE_INFO) == text_file_contents);

        // put things back
        fs.rename(temp_dir_b, temp_dir_a, KMPKG_LINE_INFO);
        REQUIRE(fs.read_contents(temp_dir_a_file, KMPKG_LINE_INFO) == text_file_contents);
        REQUIRE(!fs.exists(temp_dir_b, KMPKG_LINE_INFO));
    }

    // try rename_or_delete directory, target does not exist
    {
        REQUIRE(fs.rename_or_delete(temp_dir_a, temp_dir_b, KMPKG_LINE_INFO));
        REQUIRE(!fs.exists(temp_dir_a, KMPKG_LINE_INFO));
        REQUIRE(fs.read_contents(temp_dir_b_file, KMPKG_LINE_INFO) == text_file_contents);
    ...
    {
        fs.create_directory(temp_dir_b, KMPKG_LINE_INFO);
        fs.write_contents(temp_dir_b_file, text_file_contents, KMPKG_LINE_INFO);

        // Note that the KMPKG_LINE_INFO overload implicitly tests that ec got cleared
        REQUIRE(!fs.rename_or_delete(temp_dir_a, temp_dir_b, KMPKG_LINE_INFO));
        REQUIRE(!fs.exists(temp_dir_a, KMPKG_LINE_INFO));
        REQUIRE(fs.read_contents(temp_dir_b_file, KMPKG_LINE_INFO) == text_file_contents);
    ...
    {
        fs.create_directory(temp_dir_b, KMPKG_LINE_INFO);
        REQUIRE(fs.rename_or_delete(temp_dir_a_file, temp_dir_b_file, KMPKG_LINE_INFO));
        REQUIRE(!fs.exists(temp_dir_a_file, KMPKG_LINE_INFO));
        REQUIRE(fs.read_contents(temp_dir_b_file, KMPKG_LINE_INFO) == text_file_contents);

        // put things back
        fs.rename(temp_dir_b_file, temp_dir_a_file, KMPKG_LINE_INFO);
        REQUIRE(fs.read_contents(temp_dir_a_file, KMPKG_LINE_INFO) == text_file_contents);
        REQUIRE(!fs.exists(temp_dir_b_file, KMPKG_LINE_INFO));
        fs.remove(temp_dir_b, KMPKG_LINE_INFO);
    }
...
TEST_CASE ("copy_symlink", "[files]")
{
    ...
    fs.create_directory(temp_dir, KMPKG_LINE_INFO);
    fs.create_directory(temp_dir / "dir", KMPKG_LINE_INFO);
    fs.write_contents(temp_dir / "file", "some file contents", KMPKG_LINE_INFO);
    ...
    {
        REQUIRE(fs.read_contents(temp_dir / "dir/sym", KMPKG_LINE_INFO) == "some file contents");
        fs.copy_symlink(temp_dir / "dir/sym", temp_dir / "dir/sym_copy", KMPKG_LINE_INFO);
        REQUIRE(fs.read_contents(temp_dir / "dir/sym_copy", KMPKG_LINE_INFO) == "some file contents");
    }
...
TEST_CASE ("find_file_recursively_up", "[files]")
{
    ...
    fs.create_directory(test_root, KMPKG_LINE_INFO);
    auto one = test_root / "one";
    auto two = one / "two";
    fs.create_directory(one, KMPKG_LINE_INFO);
    fs.create_directory(two, KMPKG_LINE_INFO);
    auto one_marker = one / ".one-marker";
    fs.write_contents(one_marker, "", KMPKG_LINE_INFO);
    ...
    fs.remove_all(test_root, KMPKG_LINE_INFO);
}
...
TEST_CASE ("win32_fix_path_case", "[files]")
{
    ...
    auto original_cwd = real_filesystem.current_path(KMPKG_LINE_INFO);
    real_filesystem.current_path("C:\\", KMPKG_LINE_INFO);
    CHECK(win32_fix_path_case("\\") == "\\");
    ...
    real_filesystem.current_path(original_cwd, KMPKG_LINE_INFO);

    real_filesystem.create_directories("SuB/Dir/Ectory", KMPKG_LINE_INFO);
    ...
    CHECK(win32_fix_path_case("sub/dir/ectory/") == "SuB\\Dir\\Ectory\\");
    real_filesystem.remove_all("SuB", KMPKG_LINE_INFO);
