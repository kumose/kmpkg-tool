...
namespace kmpkg::Test
{
    ...
    {
        std::vector<kmpkg::Paragraph> pghs;
        for (auto&& p : v)
        {
            pghs.emplace_back();
            for (auto&& kv : p)
    ...
    PackageSpec PackageSpecMap::emplace(kmpkg::SourceControlFileAndLocation&& scfl)
    {
    ...
    {
#if defined(_WIN32)
        return Path(kmpkg::get_environment_variable("TEMP").value_or_exit(KMPKG_LINE_INFO)) / "kmpkg-test";
#else
        return "/tmp/kmpkg-test";
#endif
    }
    ...
    static void check_json_eq(const Json::Value& l, const Json::Value& r, std::string& path, bool ordered)
    {
        ...
        {
            check_json_eq(l.object(KMPKG_LINE_INFO), r.object(KMPKG_LINE_INFO), path, ordered);
        }
        else if (l.is_array() && r.is_array())
        {
            check_json_eq(l.array(KMPKG_LINE_INFO), r.array(KMPKG_LINE_INFO), path, ordered);
        }
        else if (l != r)
        {
            INFO(path);
