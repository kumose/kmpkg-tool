...
KMPKG_FORMAT_WITH_TO_STRING(ConstructTracker<0>);
KMPKG_FORMAT_WITH_TO_STRING(ConstructTracker<1>);
...
TEST_CASE ("move assignment value value", "[expected]")
{
...

    {
        ...
        CHECK(!originally_value.value(KMPKG_LINE_INFO).moved_from);
        CHECK(originally_value2.value(KMPKG_LINE_INFO).moved_from);
        CHECK(value.alive == 2);
...
TEST_CASE ("move assignment error value", "[expected]")
{
...

    {
        ...
        CHECK(originally_value.value(KMPKG_LINE_INFO).moved_from);
        CHECK(!originally_error.value(KMPKG_LINE_INFO).moved_from);
        error.check_nothing();
        CHECK(value.alive == 2);
        CHECK(value.copies == 0);
        CHECK(value.copy_assigns == 0);
...
TEST_CASE ("map", "[expected]")
{
    ...
    using TestType = ExpectedT<ConstructTracker<0>, ConstructTracker<1>>;
    {
        ...
        CHECK(result.value(KMPKG_LINE_INFO) == 42);
    }
