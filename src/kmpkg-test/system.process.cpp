...
TEST_CASE ("captures-output", "[system.process]")
{
    ...
    auto run = cmd_execute_and_capture_output(cmd, settings).value_or_exit(KMPKG_LINE_INFO);
...
TEST_CASE ("closes-exit-minus-one cmd_execute", "[system.process]")
{
    ...
    auto return_value = cmd_execute(Command{test_program}, settings).value_or_exit(KMPKG_LINE_INFO);
#ifdef _WIN32
...
TEST_CASE ("closes-exit-minus-one cmd_execute_and_capture_output", "[system.process]")
{
    ...
    auto run = cmd_execute_and_capture_output(Command{test_program}, settings).value_or_exit(KMPKG_LINE_INFO);
#ifdef _WIN32
