...
TEST_CASE ("contains_any_ignoring_c_comments", "[strings]")
{
    ...
    Strings::kmpkg_searcher to_find[] = {Strings::kmpkg_searcher(a.begin(), a.end()),
                                         Strings::kmpkg_searcher(b.begin(), b.end())};
...
TEST_CASE ("api_stable_format(sv,append_f)", "[strings]")
{
    ...
    FullyBufferedDiagnosticContext bdc{};
    {
        ...
        REQUIRE(bdc.empty());
        REQUIRE(res.value_or_exit(KMPKG_LINE_INFO) == "}");
    }
    {
        auto res = api_stable_format(bdc, "{{", [](std::string&, StringView) {
            CHECK(false);
    ...
    }
    {
        ...
        REQUIRE(bdc.empty());
        REQUIRE(res.value_or_exit(KMPKG_LINE_INFO) == "xxyyzz");
    }
    {
        ...
        REQUIRE(bdc.empty());
        REQUIRE(res.value_or_exit(KMPKG_LINE_INFO) == "hello}");
    }
    {
        ...
        REQUIRE(bdc.empty());
        REQUIRE(res.value_or_exit(KMPKG_LINE_INFO) == "123hello456");
    }
}
