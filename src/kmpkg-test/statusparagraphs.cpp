...
using namespace kmpkg::Paragraphs;
...
{
    REQUIRE(parse_status_line("install ok installed", test_origin, test_textrowcol).value_or_exit(KMPKG_LINE_INFO) ==
            StatusLine{Want::INSTALL, InstallState::INSTALLED});
    REQUIRE(parse_status_line("hold ok installed", test_origin, test_textrowcol).value_or_exit(KMPKG_LINE_INFO) ==
            StatusLine{Want::HOLD, InstallState::INSTALLED});
    REQUIRE(parse_status_line("deinstall ok installed", test_origin, test_textrowcol).value_or_exit(KMPKG_LINE_INFO) ==
            StatusLine{Want::DEINSTALL, InstallState::INSTALLED});
    REQUIRE(parse_status_line("purge ok installed", test_origin, test_textrowcol).value_or_exit(KMPKG_LINE_INFO) ==
            StatusLine{Want::PURGE, InstallState::INSTALLED});

    REQUIRE(
        parse_status_line("install ok not-installed", test_origin, test_textrowcol).value_or_exit(KMPKG_LINE_INFO) ==
        StatusLine{Want::INSTALL, InstallState::NOT_INSTALLED});
    REQUIRE(
        parse_status_line("install ok half-installed", test_origin, test_textrowcol).value_or_exit(KMPKG_LINE_INFO) ==
