...
TEST_CASE ("get message_map from LCID", "[messages]")
{
    ...
    auto msg = map.value_or_exit(KMPKG_LINE_INFO).map.get(msg_name);
    CHECK(msg->string(KMPKG_LINE_INFO) ==
...
TEST_CASE ("adapt DiagnosticContext to ExpectedL", "[diagnostics]")
{
    // returns_optional_prvalue
    {
        ...
        REQUIRE(adapted.value_or_exit(KMPKG_LINE_INFO) == 42);
    }
    {
        int move_limit = 1;
    ...
    // returns_optional_lvalue
    {
        ...
        REQUIRE(same_object(adapted.value_or_exit(KMPKG_LINE_INFO), the_lvalue.value_or_exit(KMPKG_LINE_INFO)));
    }
    ...
    // returns_optional_xvalue
    {
        ...
        REQUIRE(
            adapt_context_to_expected(returns_optional_xvalue, std::move(an_lvalue)).value_or_exit(KMPKG_LINE_INFO) ==
            42);
    }
    {
        int move_limit = 1;
    ...
    // returns_optional_const_xvalue
    {
        ...
        REQUIRE(adapt_context_to_expected(returns_optional_const_xvalue, std::move(an_lvalue))
                    .value_or_exit(KMPKG_LINE_INFO) == 42);
    }
    {
        bool copied = false;
    ...
    // returns_unique_ptr_lvalue
    {
        ...
        REQUIRE(same_object(adapted.value_or_exit(KMPKG_LINE_INFO), an_lvalue));
    }
    ...
    // returns_unique_ptr_xvalue
    {
        ...
        REQUIRE(adapted.value_or_exit(KMPKG_LINE_INFO).get() == the_pointer);
    }
