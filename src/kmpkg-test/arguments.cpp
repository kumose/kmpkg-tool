...
TEST_CASE ("KmpkgCmdArguments from lowercase argument sequence", "[arguments]")
{
    std::vector<std::string> t = {"--kmpkg-root",
                                  "C:\\kmpkg",
    ...
    auto v = KmpkgCmdArguments::create_from_arg_sequence(t.data(), t.data() + t.size());

    REQUIRE(v.kmpkg_root_dir_arg.value_or_exit(KMPKG_LINE_INFO) == "C:\\kmpkg");
    REQUIRE(!v.kmpkg_root_dir_env.has_value());
    REQUIRE(v.scripts_root_dir.value_or_exit(KMPKG_LINE_INFO) == "C:\\scripts");
    REQUIRE(v.builtin_ports_root_dir.value_or_exit(KMPKG_LINE_INFO) == "C:\\ports");
    REQUIRE(v.builtin_registry_versions_dir.value_or_exit(KMPKG_LINE_INFO) == "C:\\versions");
    REQUIRE(v.debug);
    REQUIRE(*v.debug.get());
...
TEST_CASE ("KmpkgCmdArguments from uppercase argument sequence", "[arguments]")
{
    std::vector<std::string> t = {"--KMPKG-ROOT",
                                  "C:\\kmpkg",
                                  "--X-SCRIPTS-ROOT=C:\\scripts",
...
TEST_CASE ("KmpkgCmdArguments from argument sequence with valued options", "[arguments]")
{
    SECTION ("case 1")
    {
...
TEST_CASE ("kmpkg_root parse with arg separator", "[arguments]")
{
    std::vector<std::string> t = {"--kmpkg-root", "C:\\kmpkg"};
    auto v = KmpkgCmdArguments::create_from_arg_sequence(t.data(), t.data() + t.size());
    REQUIRE(v.kmpkg_root_dir_arg.value_or_exit(KMPKG_LINE_INFO) == "C:\\kmpkg");
}

TEST_CASE ("kmpkg_root parse with equal separator", "[arguments]")
{
    std::vector<std::string> t = {"--kmpkg-root=C:\\kmpkg"};
    auto v = KmpkgCmdArguments::create_from_arg_sequence(t.data(), t.data() + t.size());
    REQUIRE(v.kmpkg_root_dir_arg.value_or_exit(KMPKG_LINE_INFO) == "C:\\kmpkg");
...
TEST_CASE ("Combine asset cache params", "[arguments]")
{
    ...
    v = KmpkgCmdArguments::create_from_arg_sequence(t.data(), t.data() + t.size());
    REQUIRE(v.asset_sources_template() == "x-azurl,value");

    std::map<StringLiteral, std::string, std::less<>> envmap = {
        {EnvironmentVariableXKmpkgAssetSources, "x-azurl,value1"},
    };
    v = KmpkgCmdArguments::create_from_arg_sequence(nullptr, nullptr);
...
TEST_CASE ("CMake debugger flags", "[arguments]")
{
    std::vector<std::string> t = {"--x-cmake-debug",
                                  ...
                                  "\\\\.\\pipe\\configure-pipe"};
    auto v = KmpkgCmdArguments::create_from_arg_sequence(t.data(), t.data() + t.size());
    auto& cmake_debug = v.cmake_debug.value_or_exit(KMPKG_LINE_INFO);
    ...
    auto& cmake_configure_debug = v.cmake_configure_debug.value_or_exit(KMPKG_LINE_INFO);
