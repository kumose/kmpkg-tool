...
namespace
{
    ...
    Json::Value parse_json(StringView sv) { return Json::parse(sv, "test").value(KMPKG_LINE_INFO).value; }
}
...
TEST_CASE ("check valid package patterns", "[registries]")
{
    ...
    CHECK(ID::is_ident("kmpkg-boost-helper"));
...
static kmpkg::Optional<Configuration> visit_default_registry(Json::Reader& r, Json::Value&& reg)
{
    Json::Object config;
...
TEST_CASE ("registries ignored patterns warning", "[registries]")
{
    ...
    "registries": [
        ...
        {
            "kind": "git",
            "repository": "https://github.com/another-remote/another-kmpkg-registry",
            "baseline": "bbbb0000",
            "packages": [ "*", "bei*", "zlib" ]
        }
    ]
    ...
    auto reg = regs[0];
    CHECK(reg.kind == "git");
    CHECK(reg.repo == "https://github.com/kumose/kmpkg");
    ...
    reg = regs[1];
    CHECK(reg.kind == "git");
    CHECK(reg.repo == "https://github.com/northwindtraders/kmpkg-registry");
    ...
    reg = regs[2];
    CHECK(reg.kind == "git");
    CHECK(reg.repo == "https://github.com/another-remote/another-kmpkg-registry");
...
[
    ...
    auto& results = results_opt.value_or_exit(KMPKG_LINE_INFO);
...
[
    ...
    },
    {
        ...
        CHECK(results[0].p == "a/b" KMPKG_PREFERRED_SEPARATOR "c/d");
        CHECK(results[1].version == SchemedVersion{VersionScheme::String, {"doggies", 0}});
        CHECK(results[1].p == "a/b" KMPKG_PREFERRED_SEPARATOR "e/d");
        CHECK(results[2].version == SchemedVersion{VersionScheme::Semver, {"1.2.3", 0}});
        CHECK(results[2].p == "a/b" KMPKG_PREFERRED_SEPARATOR "semvers/here");
        CHECK(!r.messages().any_errors());
    }
...
[
    {
        ...
        CHECK(filesystem_version_db.visit(r, test_json).value_or_exit(KMPKG_LINE_INFO).empty());
        CHECK(r.messages().any_errors());
    }
...
TEST_CASE ("get_all_port_names", "[registries]")
{
    ...
    SECTION ("with default registry")
    {
        ...
        CHECK(with_default_registry.get_all_reachable_port_names().value_or_exit(KMPKG_LINE_INFO) ==
        ...
        CHECK(with_default_registry.get_all_known_reachable_port_names_no_network().value_or_exit(KMPKG_LINE_INFO) ==
              std::vector<std::string>{
    ...
    SECTION ("without default registry")
    {
        ...
        // hello, world, abcdefg, abc, abcde from the first registry
        // twoRegistry from the second registry
        CHECK(without_default_registry.get_all_reachable_port_names().value_or_exit(KMPKG_LINE_INFO) ==
              std::vector<std::string>{"abc", "abcde", "abcdefg", "hello", "twoRegistry", "world"});

        // hello, world, notpresent from the first registry
        // twoOld from the second registry
        CHECK(without_default_registry.get_all_known_reachable_port_names_no_network().value_or_exit(KMPKG_LINE_INFO) ==
