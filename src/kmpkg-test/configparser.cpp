...
TEST_CASE ("BinaryConfigParser files provider", "[binaryconfigparser]")
{
    ...
    }
    {
        ...
        auto state = parsed.value_or_exit(KMPKG_LINE_INFO);
...
TEST_CASE ("AssetConfigParser azurl provider", "[assetconfigparser]")
{
    ...
    }
    {
        AssetCachingSettings dm =
            parse_download_configuration("x-azurl,https://abc/123,foo").value_or_exit(KMPKG_LINE_INFO);
    ...
    {
        AssetCachingSettings dm =
            parse_download_configuration("x-azurl,https://abc/123").value_or_exit(KMPKG_LINE_INFO);
    ...
    {
        AssetCachingSettings dm =
            parse_download_configuration("x-azurl,https://abc/123,,readwrite").value_or_exit(KMPKG_LINE_INFO);
        CHECK(dm.m_read_url_template == "https://abc/123/<SHA>");
        CHECK(dm.m_read_headers.empty());
    ...
    {
        AssetCachingSettings dm =
            parse_download_configuration("x-azurl,https://abc/123,foo,readwrite").value_or_exit(KMPKG_LINE_INFO);
        CHECK(dm.m_read_url_template == "https://abc/123/<SHA>?foo");
        CHECK(dm.m_read_headers.empty());
    ...
    {
        AssetCachingSettings dm =
            parse_download_configuration("x-script,powershell {SHA} {URL}").value_or_exit(KMPKG_LINE_INFO);
        ...
        CHECK(dm.m_script.value_or_exit(KMPKG_LINE_INFO) == "powershell {SHA} {URL}");
    }
}
