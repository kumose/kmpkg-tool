...
namespace
{
    auto test_parse_control_file(const std::vector<std::unordered_map<std::string, std::string>>& v)
    {
        ...
        for (auto&& p : v)
        {
            pghs.emplace_back();
            for (auto&& kv : p)
                pghs.back().emplace(kv.first, std::make_pair(kv.second, kmpkg::TextRowCol{}));
        }
        return kmpkg::SourceControlFile::parse_control_file("test-origin", std::move(pghs));
    }
    ...
    auto test_make_binary_paragraph(const std::unordered_map<std::string, std::string>& v)
    {
        Paragraph pgh;
        for (auto&& kv : v)
            pgh.emplace(kv.first, std::make_pair(kv.second, kmpkg::TextRowCol{}));

        return kmpkg::BinaryParagraph("test", std::move(pgh));
    }
...
TEST_CASE ("BinaryParagraph dependencies with triplets", "[paragraph]")
{
    ...
    REQUIRE(pgh.dependencies.size() == 3);
    REQUIRE(pgh.dependencies[0].name() == "a");
    REQUIRE(pgh.dependencies[0].triplet() == kmpkg::Test::X64_WINDOWS);
    REQUIRE(pgh.dependencies[1].name() == "b");
    REQUIRE(pgh.dependencies[1].triplet() == kmpkg::Test::X86_WINDOWS);
    REQUIRE(pgh.dependencies[2].name() == "c");
    REQUIRE(pgh.dependencies[2].triplet() == kmpkg::Test::ARM_UWP);
}
...
{
    const char* str = "";
    auto pghs = kmpkg::Paragraphs::parse_paragraphs(str, "test-origin").value_or_exit(KMPKG_LINE_INFO);
    REQUIRE(pghs.empty());
}
...
TEST_CASE ("BinaryParagraph serialize min", "[paragraph]")
{
    ...
    auto pghs = kmpkg::Paragraphs::parse_paragraphs(ss, "test-origin").value_or_exit(KMPKG_LINE_INFO);
