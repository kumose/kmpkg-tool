...
{
    {
        ...
        replace_response_file_parameters(empty, NeverReadLines{}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(empty == std::vector<std::string>{});
    }

    {
        std::vector<std::string> no_responses{"a", "b", "c"};
        replace_response_file_parameters(no_responses, NeverReadLines{}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(no_responses == std::vector<std::string>{"a", "b", "c"});
    }

    {
        std::vector<std::string> remove_only{"@filename"};
        replace_response_file_parameters(remove_only, FakeReadLines{}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(remove_only == std::vector<std::string>{});
    }

    {
        std::vector<std::string> remove_first{"@filename", "a", "b"};
        replace_response_file_parameters(remove_first, FakeReadLines{}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(remove_first == std::vector<std::string>{"a", "b"});
    }

    {
        std::vector<std::string> remove_middle{"a", "@filename", "b"};
        replace_response_file_parameters(remove_middle, FakeReadLines{}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(remove_middle == std::vector<std::string>{"a", "b"});
    }

    {
        std::vector<std::string> remove_last{"a", "b", "@filename"};
        replace_response_file_parameters(remove_last, FakeReadLines{}).value_or_exit(KMPKG_LINE_INFO);
    ...
    const std::vector<std::string> only_x{"x"};
    {
        ...
        replace_response_file_parameters(insert_only, FakeReadLines{only_x}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(insert_only == std::vector<std::string>{"x"});
    }

    {
        std::vector<std::string> insert_first{"@filename", "a", "b"};
        replace_response_file_parameters(insert_first, FakeReadLines{only_x}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(insert_first == std::vector<std::string>{"x", "a", "b"});
    }

    {
        std::vector<std::string> insert_middle{"a", "@filename", "b"};
        replace_response_file_parameters(insert_middle, FakeReadLines{only_x}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(insert_middle == std::vector<std::string>{"a", "x", "b"});
    }

    {
        std::vector<std::string> insert_last{"a", "b", "@filename"};
        replace_response_file_parameters(insert_last, FakeReadLines{only_x}).value_or_exit(KMPKG_LINE_INFO);
    ...
    const std::vector<std::string> xy{"x", "y"};
    {
        ...
        replace_response_file_parameters(multi_insert_only, FakeReadLines{xy}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(multi_insert_only == std::vector<std::string>{"x", "y"});
    }

    {
        std::vector<std::string> multi_insert_first{"@filename", "a", "b"};
        replace_response_file_parameters(multi_insert_first, FakeReadLines{xy}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(multi_insert_first == std::vector<std::string>{"x", "y", "a", "b"});
    }

    {
        std::vector<std::string> multi_insert_middle{"a", "@filename", "b"};
        replace_response_file_parameters(multi_insert_middle, FakeReadLines{xy}).value_or_exit(KMPKG_LINE_INFO);
        CHECK(multi_insert_middle == std::vector<std::string>{"a", "x", "y", "b"});
    }

    {
        std::vector<std::string> multi_insert_last{"a", "b", "@filename"};
        replace_response_file_parameters(multi_insert_last, FakeReadLines{xy}).value_or_exit(KMPKG_LINE_INFO);
    ...
    const std::vector<std::string> blanks{"x", "", "    ", "\r", "y"};
    {
        ...
        replace_response_file_parameters(insert_middle_blanks, FakeReadLines{blanks}).value_or_exit(KMPKG_LINE_INFO);
...
TEST_CASE ("Arguments can be parsed as switches", "[cmd_parser]")
{
    ...
    CHECK(!uut.parse_switch("unset-switch", StabilityTag::Standard, optional_switch_value));
    CHECK(optional_switch_value.value_or_exit(KMPKG_LINE_INFO) == false);
    optional_switch_value.clear();
    CHECK(uut.parse_switch("optional_switch", StabilityTag::Standard, optional_switch_value));
    CHECK(optional_switch_value.value_or_exit(KMPKG_LINE_INFO) == true);
    optional_switch_value.emplace(false);
    ...
    CHECK(opt_disabled_switch.value_or_exit(KMPKG_LINE_INFO) == false);
    ...
    CHECK(uut.parse_switch_combined("simple", StabilityTag::Standard));
...
TEST_CASE ("Options can be parsed", "[cmd_parser]")
{
    ...
    CHECK(!uut.parse_option("unset-option", StabilityTag::Standard, optional_option_value));
    CHECK(optional_option_value.value_or_exit(KMPKG_LINE_INFO) == "default value");
    optional_option_value.clear();
    CHECK(uut.parse_option("optional-value", StabilityTag::Standard, optional_option_value));
    CHECK(optional_option_value.value_or_exit(KMPKG_LINE_INFO) == "set");
...
TEST_CASE ("Options missing values in the middle generate errors", "[cmd_parser]")
{
    ...
    v.emplace_back("a");
    CmdParser uut{v};
    CHECK(uut.parse_switch_combined("switch", StabilityTag::Standard));
    std::string value;
    CHECK(!uut.parse_option("missing-value", StabilityTag::Standard, value));
    CHECK(value.empty());
    CHECK(uut.get_errors() == localized({"error: the option 'missing-value' requires a value"}));
...
TEST_CASE ("Multi-options can be parsed", "[cmd_parser]")
{
    ...
    CHECK(!uut.parse_multi_option("unset-option", StabilityTag::Standard, optional_option_value));
    CHECK(optional_option_value.value_or_exit(KMPKG_LINE_INFO) == std::vector<std::string>{"default value"});
    optional_option_value.clear();
    CHECK(uut.parse_multi_option("optional-value", StabilityTag::Standard, optional_option_value));
    CHECK(optional_option_value.value_or_exit(KMPKG_LINE_INFO) == std::vector<std::string>{"set", "set2"});
    optional_option_value.emplace(std::vector<std::string>{"default value"});
    CHECK(uut.parse_multi_option("optional-defaulted-value", StabilityTag::Standard, optional_option_value));
    CHECK(optional_option_value.value_or_exit(KMPKG_LINE_INFO) == std::vector<std::string>{"set"});
...
TEST_CASE ("Help table is generated", "[cmd_parser]")
{
    ...
    uut.parse_switch_combined("x", StabilityTag::Standard, LocalizedString::from_raw("x help"));
    uut.parse_switch_combined("f", StabilityTag::Experimental, LocalizedString::from_raw("f help"));
...
TEST_CASE ("Consume zero or one remaining args", "[cmd_parser]")
{
    ...
    {
        CmdParser uut{std::vector<std::string>{"first-arg"}};
        CHECK(uut.consume_only_remaining_arg_optional("example").value_or_exit(KMPKG_LINE_INFO) == "first-arg");
        CHECK(uut.get_errors().empty());
        CHECK(uut.get_remaining_args().empty());
    }
...
{
    {
        ...
        CHECK(uut.extract_first_command_like_arg_lowercase().value_or_exit(KMPKG_LINE_INFO) == "depend-info");
        CHECK(uut.get_errors().empty());
...

    {
        ...
        CHECK(uut.parse_switch_combined("dot", StabilityTag::Standard));
        CHECK(uut.consume_only_remaining_arg("depend-info").empty());
        CHECK(uut.get_errors() == localized({"error: the option 'sort' requires a value; if you intended to set 'sort' "
                                             "to '--dot', use the equals form instead: --sort=--dot",
                                             "error: unexpected switch: --x-tree",
...

    {
        ...
        CHECK(uut.extract_first_command_like_arg_lowercase().value_or_exit(KMPKG_LINE_INFO) == "version");
    }
    ...
    {
        CmdParser uut{std::vector<std::string>{"/?"}};
        CHECK(uut.extract_first_command_like_arg_lowercase().value_or_exit(KMPKG_LINE_INFO) == "help");
    }
...
TEST_CASE ("inverted switches", "[cmd_parser]")
{
    // Support for existing switches that already start with "no-"
    {
        ...
        CHECK(uut.parse_switch_combined("no-switch", StabilityTag::Standard));
        CHECK(uut.get_errors().empty());
        CHECK(uut.get_remaining_args().empty());
    }
...

    {
        ...
        CHECK(uut.parse_switch_combined("no-switch", StabilityTag::Experimental));
...
TEST_CASE ("Options do not return consumed args", "[cmd_parser]")
{
    ...
    CHECK(uut.extract_first_command_like_arg_lowercase().value_or_exit(KMPKG_LINE_INFO) == "command");
