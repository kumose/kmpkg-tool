...
{
    template<>
    struct StringMaker<kmpkg::DeploymentKind>
    {
        static const std::string convert(const kmpkg::DeploymentKind& value)
        {
...
{
    auto result = try_parse_bundle_settings({"{}", "test"}).value_or_exit(KMPKG_LINE_INFO);
...
TEST_CASE ("parse-all-fields", "[bundle-settings]")
{
    ...
    CHECK(result.embedded_git_sha.value_or_exit(KMPKG_LINE_INFO) == "a7a6d5edaff9d850db2d5f1378e5d9af59805e81");
    CHECK(result.deployment == DeploymentKind::OneLiner);
    CHECK(result.vsversion.value_or_exit(KMPKG_LINE_INFO) == "16.0");
}
