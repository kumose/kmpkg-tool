...
{
    std::vector<std::unique_ptr<StatusParagraph>> status_paragraphs;
    status_paragraphs.push_back(kmpkg::Test::make_status_pgh("a"));
...
TEST_CASE ("install platform dependent default features", "[plan]")
{
    ...
    auto linux_expr = PlatformExpression::parse_platform_expression("linux", MBO::Deny).value_or_exit(KMPKG_LINE_INFO);
    ...
    SECTION ("on linux")
    {
        var_provider.dep_info_vars[iter] = {{"KMPKG_CMAKE_SYSTEM_NAME", "Linux"}};
        auto install_plan = create_feature_install_plan(map_port,
                                                        var_provider,
                                                        Test::parse_test_fspecs("a:x64-windows"),
                                                        StatusParagraphs(std::move(status_paragraphs)));
