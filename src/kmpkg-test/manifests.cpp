...
static Json::Object parse_json_object(StringView sv)
{
    ...
    if (auto r = json.get())
    {
        return std::move(r->value.object(KMPKG_LINE_INFO));
    }
    else
    {
...
TEST_CASE ("manifest versioning", "[manifests]")
{
    ...
    for (auto&& v : data)
    {
        auto portManifest = Json::parse_object(std::get<0>(v), "test").value_or_exit(KMPKG_LINE_INFO);
        { // project manifest
...
TEST_CASE ("manifest embed configuration", "[manifests]")
{
    std::string raw_config = R"json({
        ...
        "registries": [
            ...
            {
                "kind": "git",
                "repository": "https://github.com/kumose/kmpkg-ports",
                "baseline": "089fa4de7dca22c67dcab631f618d5cd0697c8d4",
                "packages": [ 
                    "zlib",
                    "rapidjson",
    ...
    auto config = Json::parse(raw_config, "<test config>").value(KMPKG_LINE_INFO).value;
    REQUIRE(config.is_object());
    auto config_obj = config.object(KMPKG_LINE_INFO);
    REQUIRE(pgh.core_paragraph->configuration.has_value());
...
TEST_CASE ("manifest embed kmpkg_configuration", "[manifests]")
{
    std::string raw_config = R"json({
        "$extra-info": null,
        "default-registry": {
    ...
    auto parsed_config_obj = *pgh.core_paragraph->configuration.get();
    REQUIRE(Json::stringify(parsed_config_obj) == Json::stringify(config_obj));
    REQUIRE(pgh.core_paragraph->configuration_source == ConfigurationSource::ManifestFileKmpkgConfiguration);
}
...
TEST_CASE ("manifest construct maximum", "[manifests]")
{
    ...
    auto contact_a_aa = contact_a->object(KMPKG_LINE_INFO).get("aa");
    REQUIRE(contact_a_aa);
    REQUIRE(contact_a_aa->is_string());
    REQUIRE(contact_a_aa->string(KMPKG_LINE_INFO) == "aa");
    ...
    REQUIRE(
        pgh.feature_paragraphs[0]->dependencies[1].features[1].platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", ""}}));
    ...
    REQUIRE_FALSE(pgh.feature_paragraphs[0]->dependencies[1].platform.evaluate(
        {{"KMPKG_CMAKE_SYSTEM_NAME", ""}, {"KMPKG_TARGET_ARCHITECTURE", "arm"}}));
    REQUIRE(pgh.feature_paragraphs[0]->dependencies[1].platform.evaluate(
        {{"KMPKG_CMAKE_SYSTEM_NAME", ""}, {"KMPKG_TARGET_ARCHITECTURE", "x86"}}));
    REQUIRE(pgh.feature_paragraphs[0]->dependencies[1].platform.evaluate(
        {{"KMPKG_CMAKE_SYSTEM_NAME", "Linux"}, {"KMPKG_TARGET_ARCHITECTURE", "x86"}}));
...
TEST_CASE ("SourceParagraph manifest construct qualified dependencies", "[manifests]")
{
    ...
    REQUIRE(pgh.core_paragraph->dependencies[0].platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", ""}}));
    REQUIRE(pgh.core_paragraph->dependencies[1].name == "libb");
    REQUIRE(pgh.core_paragraph->dependencies[1].platform.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", "WindowsStore"}}));
}
...
TEST_CASE ("SourceParagraph manifest supports", "[manifests]")
{
    ...
    REQUIRE(pgh.core_paragraph->supports_expression.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", "Linux"}}));
    REQUIRE_FALSE(pgh.core_paragraph->supports_expression.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", ""}}));
    REQUIRE_FALSE(pgh.core_paragraph->supports_expression.evaluate({{"KMPKG_CMAKE_SYSTEM_NAME", "Darwin"}}));
}
...
static std::string test_serialized_license(StringView license)
{
    ...
    return serialize_manifest(**m_pgh.get())["license"].string(KMPKG_LINE_INFO).to_string();
}
