# Outputs to Cache: KMPKG_COMPILER
function(kmpkg_detect_compiler)
    if(NOT DEFINED CACHE{KMPKG_COMPILER})
...
On CentOS try the following:
    ...
    scl enable devtoolset-7 bash
        ...
        set(KMPKG_COMPILER ${COMPILER}
            CACHE STRING
            "The compiler in use; one of gcc, clang, msvc")
        message(STATUS "Detecting the C++ compiler in use - ${KMPKG_COMPILER}")
    endif()
endfunction()

function(kmpkg_target_add_sourcelink target)
    cmake_parse_arguments(PARSE_ARGV 1 "arg" "" "REPO;REF" "")
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "kmpkg_cmake_buildsystem_build was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()
    ...
    if(MSVC)
        ...
        set(output_json_filename "${CMAKE_CURRENT_BINARY_DIR}/kmpkgsourcelink.json")
        # string(JSON was added in CMake 3.19, so just create the json from scratch
        file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/kmpkgsourcelink.json"
"{
  \"documents\": {
    \"${local_base}\": \"${base_url}\"
  }
