...
namespace kmpkg
{
    ...
    struct DiagnosticLine
    {
            ...
            : m_kind(kind), m_origin(origin.to_string()), m_position(), m_message(std::forward<MessageLike>(message))
        {
            if (origin.empty())
            {
                Checks::unreachable(KMPKG_LINE_INFO, "origin must not be empty");
            }
        }
    ...
    struct DiagnosticContext
    {
        ...
        void report_error(KMPKG_DECL_MSG_ARGS)
        {
            LocalizedString message;
            msg::format_to(message, KMPKG_EXPAND_MSG_ARGS);
            this->report_error(std::move(message));
        }

        template<KMPKG_DECL_MSG_TEMPLATE>
        void report_error_with_log(StringView log_content, KMPKG_DECL_MSG_ARGS)
        {
            LocalizedString message;
            msg::format_to(message, KMPKG_EXPAND_MSG_ARGS);
            message.append_raw('\n');
