...
namespace kmpkg
{
    ...
    struct KmpkgCmdArguments
    {
        static KmpkgCmdArguments create_from_command_line(const ILineReader& fs,
                                                          const int argc,
                                                          const CommandLineCharType* const* const argv);
        static KmpkgCmdArguments create_from_arg_sequence(const std::string* arg_begin, const std::string* arg_end);

        Optional<std::string> kmpkg_root_dir_arg;
        Optional<std::string> kmpkg_root_dir_env;
        ...
        Optional<std::string> kmpkg_nuget_repository;
        ...
        static void imbue_or_apply_process_recursion(KmpkgCmdArguments& args);
    ...
    private:
        KmpkgCmdArguments(CmdParser&& parser);

        void imbue_from_environment_impl(std::function<Optional<std::string>(ZStringView)> get_env);

        Optional<std::string> asset_sources_template_env; // for EnvironmentVariableXKmpkgAssetSources
