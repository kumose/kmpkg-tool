...
#include <kmpkg/base/fwd/parse.h>
...
template<>
struct std::hash<kmpkg::PackageSpec>
{
    size_t operator()(const kmpkg::PackageSpec& value) const
    {
        size_t hash = 17;
        hash = hash * 31 + std::hash<std::string>()(value.name());
        hash = hash * 31 + std::hash<kmpkg::Triplet>()(value.triplet());
        return hash;
    }
};

template<>
struct std::hash<kmpkg::FeatureSpec>
{
    size_t operator()(const kmpkg::FeatureSpec& value) const
    {
        size_t hash = std::hash<kmpkg::PackageSpec>()(value.spec());
        hash = hash * 31 + std::hash<std::string>()(value.feature());
        return hash;
    }
};

KMPKG_FORMAT_WITH_TO_STRING(kmpkg::PackageSpec);
KMPKG_FORMAT_WITH_TO_STRING(kmpkg::FeatureSpec);
KMPKG_FORMAT_WITH_TO_STRING(kmpkg::FullPackageSpec);
